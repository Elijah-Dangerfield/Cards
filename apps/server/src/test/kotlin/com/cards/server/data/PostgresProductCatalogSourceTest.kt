...
class PostgresProductCatalogSourceTest : DatabaseTest() {
    ...
    fun read_returnsKnownProductsThatClientReferencesByGrantsKey() = runTest {
        ...
        val expectedGrantKeys = setOf(
            ...
            "emotes.fierce",
            "emotes.royal",
            // The three titles are unlock-only as of V51, and the table.sunset
            // (V64) + table.neon (V70) table themes were removed outright
            // (redundant with the plain felts) — none are in the GET /v1/products
            // catalog, so they're intentionally absent here.
        )
