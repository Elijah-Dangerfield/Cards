...
- **"The app crashed" reports — verify with the crash marker:** the *next* `app.launched` from the same install carries `previous_exit` (`crash`/`anr`/`oom`/`clean`/`unknown`):
  `logql='{service_name="cards-client", deployment_environment="prod"} | install_id="<id>" | event_name="app.launched"'` over a window past the report. `previous_exit="crash"` confirms a hard crash even when Sentry caught nothing; **iOS always reports `unknown`** (MetricKit not wired yet — ENG-25), so absence of `crash` on iOS proves nothing.
...
- **Absence of client events is weak evidence:** delivery is at-most-once — events emitted while the device was offline are dropped (ENG-25), and `app.launched` carries its own orphan `session_id` (fires before the first-foreground rollover — ENG-24), so a session's trail won't include its launch event. Don't conclude "the user never did X" from a missing client event alone; corroborate with server logs or breadcrumbs.
