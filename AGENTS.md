...
- **Build the best thing, not the smallest change.** Android (Play) and iOS (App Store since 2026-07-23) are both live, with real users and real data. The goal is systems that are **scalable, maintainable, and production-ready** — restructure or replace rather than stacking patches — but both platforms now carry a production population, so schema / persisted-state / live-behaviour changes must be safe for existing users on either one. Before implementing, take a step back and ask what's genuinely best for the project and the user experience — and if the right answer is to restructure, replace, or rebuild something already here, do that rather than stacking a minimal patch on top of what exists (that's how a codebase rots into "trash stacked on trash"). Surface the cheaper option if one exists, but recommend the best one and say why. Minimal-first is for hotfixes or when the owner explicitly asks for speed — it is never the default.
...
- Never use backslash escapes or em dashes in `strings.xml`: Compose-MP renders `\'` literally so write `it's` not `it\'s` (`\n` is fine), and use a hyphen or rephrase instead of an em dash. **No em dashes anywhere in user-facing copy** — the em dash is the single most legible AI tell; rephrase or use a comma/period.
