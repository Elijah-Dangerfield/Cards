...
- **Build the best thing, not the smallest change.** This is an unshipped, greenfield codebase. When the right fix means restructuring, replacing, or rebuilding something already here, do that rather than stacking a minimal patch on top of what exists. Surface the cheaper option if one exists, but recommend the one that's genuinely best for the codebase and the user experience, and say why. Minimal-first is for hotfixes or when the owner explicitly asks for speed — it is not the default.
