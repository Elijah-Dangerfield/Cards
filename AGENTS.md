...
- **Build the best thing, not the smallest change.** Android is live (real users, real data); iOS isn't shipped yet. The goal is systems that are **scalable, maintainable, and production-ready** — restructure or replace rather than stacking patches — but Android now has a production population, so schema / persisted-state / live-behaviour changes must be safe for existing users (iOS has none yet). Before implementing, take a step back and ask what's genuinely best for the project and the user experience — and if the right answer is to restructure, replace, or rebuild something already here, do that rather than stacking a minimal patch on top of what exists (that's how a codebase rots into "trash stacked on trash"). Surface the cheaper option if one exists, but recommend the best one and say why. Minimal-first is for hotfixes or when the owner explicitly asks for speed — it is never the default.
