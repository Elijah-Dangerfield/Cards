...
- **Build the best thing, not the smallest change.** Greenfield cuts both ways: it removes "hard to undo" as an excuse to skip, AND it removes "smallest diff" as the default. When the right fix means restructuring or replacing existing code, do that — don't stack a minimal patch on top to avoid touching what's there (that's how a codebase rots). Pick the solution that's genuinely best for the codebase and the user, even when it's more work. Minimal-first is for a hotfix, not the norm.
