...
## fix: don't crash the play screen on a rejected/timed-out intent

**Problem:** `PlayPokerAction.Submit` ran `viewModelScope.launch { session.submit(action.intent) }` with no exception handling, but `RemotePokerSession.submit` *throws* — `IntentRejectedException` on a rejected ack (server says "not your turn" / illegal action, which happens routinely on a double-tap or any turn-race) and `IntentTimeoutException` after 10s with no ack. The throw escaped the launched coroutine into `viewModelScope` (a `SupervisorJob` with no `CoroutineExceptionHandler`) → uncaught → app crash on Android. Only the *accepted* submit path was tested through the VM, so the crash hid.
**Approach:** Wrapped the call in `Catching { session.submit(...) }.onFailure { logger.w(...) }` — the repo's idiom (mirrors `RemotePokerSession.pumpNextHandSignals`). A rejected/expired intent is now a logged no-op; the UI re-derives legal actions from the next snapshot, so the user simply keeps playing. Confirmed the two new VM-seam tests (`submitRejectedByServer_…`, `submitTimesOut_…`) FAIL against the pre-fix code (the uncaught throw does surface through `runTest`) and pass after — real regression guards, not vacuous "doesn't crash" assertions.
**Reviewer notes:** Bots path unaffected — `LocalBotsSession.submit` doesn't reject. I deliberately did NOT surface the rejection to the user yet (no toast/feedback): `TurnFeedback` is a haptic setting, not an error channel, and a proper "illegal action" surface wants a new state field/event — scope creep on a crash fix. The `RemotePokerSession` KDoc already promises "the VM maps this to a UI-level 'not your turn' surface"; that mapping still doesn't exist, it just no longer crashes. Verified `:features:room:impl:testDebugUnitTest`, `compileTestKotlinIosSimulatorArm64`, `:apps:compose:assembleDebug`.
**Deferred:** A user-facing "illegal action / not your turn" surface on intent rejection — nothing added to a doc; reviewer, triage whether it's worth a `docs/todo.md` follow-up (the KDoc promise is the natural home for the requirement).
