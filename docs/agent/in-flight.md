...
## fix(privacy): correct anonymous-account durability claim (AUTH-9)

**Problem:** privacy.html claimed an anonymous account lets chips/items/progress "survive an app reinstall or a phone swap" — false (reinstall/device-swap = fresh anon account; claim is the only durable path), and it contradicted our own terms.html.
**Approach:** rewrote that line to say the account is device-local, won't auto-move to a new phone or survive uninstall, and that claiming via email/Apple/Google is the only way to carry it over — now consistent with terms.html and the account-lifecycle wiki. Also fixed the two flagged minor overstatements (narrowed "hosted in the United States" to Supabase+Fly, since Sentry/Grafana regions aren't pinned in-repo; swapped the telemetry example to real events "app opened / room joined / purchase completed") and added the two suggested disclosures: `install_id` is stored on the profile row, and abandoned anonymous accounts may be auto-reaped.
**Reviewer notes:** Docs-only, no code/tests. Deliberately added no fairness/randomness wording (server uses `Random.Default`, not a CSPRNG), per the item's note. Note: `game.started` does in fact exist as an event, but I used the item's suggested `room.joined` since it's unambiguously real and reads naturally.
