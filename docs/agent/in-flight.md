...
## docs(decisions): log ws_send span-link + TracedGameEvent envelope call

**Problem:** The span-link-vs-reparent + envelope-on-events-flow choice above is a non-trivial architectural call; AGENTS.md asks those go in `docs/decisions.md`.
**Approach:** Added a 2026-05-30 entry recording the decision (span links for async fan-out, envelope to keep tracing off domain types, state-snapshot leg deferred). Pairs with the `feat` commit above — not a separate work item.
**Reviewer notes:** None.
