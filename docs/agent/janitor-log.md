...
| 2026-07-13 | features/rooms/impl/.../PublicCommon.kt | Deleted 3 dead `internal` helpers left from the pre-matchmaking shells (`ReadOnlyStakeCard`, `lobbySeats()`, `nextRoundSeats()` — hardcoded placeholder avatars, no callers module-wide); dropped now-unused imports; fixed stale `PublicHeroCard` KDoc (named removed Lobby/NextRound cards) |
...
| 2026-09-14 | libraries/identity/impl/.../auth/SecureSessionManager.kt | FIX: the legacy plaintext migration wrote the secure store directly and never seeded the AUTH-19 mirror, so a guest who upgraded off the old store had no mirror until supabase happened to refresh. Both writes now go through one `persist` |
| 2026-09-14 | libraries/identity/impl/.../auth/SupabaseAuthRepositoryImpl.kt | `resolveAndEmitLocked` hand-rolled `emitAuthenticatedFromGatewayLocked` (dup log line, missing `tokenInvalidator.invalidate()`) → calls the helper; ktor `ClientRequestException`/`ServerResponseException` FQN → imports; section banners, log-level caption + `emitUnauthenticatedLocked` narration deleted; stale resolve-exhaustion comment corrected |
| 2026-09-14 | libraries/identity/.../auth/AuthRepository.kt | Stale `signOut()` KDoc still promised a fresh anonymous sign-in on the next `current()` — untrue since auto-anon was removed |
| 2026-09-14 | libraries/identity/.../auth/GuestAccountCreator.kt | `ensureSession` KDoc linked `[healMutex]`, a private field of an impl in another module |
| 2026-09-14 | libraries/identity/impl/.../auth/SecureSessionManagerTest.kt | +2 tests: anonymous legacy migration seeds the mirror (the red repro), claimed legacy migration leaves none |
| 2026-09-14 | libraries/identity/impl/.../auth/RestExceptions.kt | NEW — shared `restException(status)` MockEngine helper; a real `RestException` needs a live Ktor response |
| 2026-09-14 | libraries/identity/impl/.../auth/GatewayAuthTokenProviderTest.kt | +3 tests: the AuthRejected branch (untested — it's what tears a session down) incl. `wasAnonymous` routing, 5xx keeps the session, `isAnonymousSession` across guest/claimed/none |
| 2026-09-14 | libraries/identity/impl/.../auth/RefreshFailureClassifierTest.kt | +2 tests: real 401/403 → AuthRejected and 500/429 → Transient; KDoc claiming the branch was unsynthesizable was stale (ktor-client-mock is already a test dep) |
| 2026-09-14 | libraries/identity/impl/.../auth/SupabaseAuthRepositoryImplTest.kt | 11 fully-qualified outcome types + `AuthTokenInvalidator` → imports; import block re-ordered |
| 2026-09-14 | libraries/identity/impl/build.gradle.kts | Test-only ktor-client-mock was hardcoded at "3.3.3", a minor behind the catalog's ktor ref (3.5.0) → `libs.ktor.client.mock` |
| 2026-09-14 | docs/agent/ai-style-guide.md | +8 tips: one-write-function for paired stores, inline hand-rolled helper duplicates, no section banners, stale "untestable" docs, no private-field KDoc links from public APIs, test the destructive branch first, catalog test deps |
