...
| 2026-09-06 | features/room/impl/.../TableUiState.kt | FIX: heads-up `blindSeats` skipped the engine's not-in-hand-button fallback, so a sat-out button seat got the SB badge and the real SB got the BB one; dropped a `winnerSeats.isEmpty()` re-check a prior guard already covers |
| 2026-09-06 | features/room/impl/.../PlayPokerContract.kt | 8 inline FQNs (`CardBackStyle`, `ProductCatalog`, `Product.ChipPack`, `IapPurchaseOutcome`, an already-imported `MatchOverCountdown`) → imports; deleted the "extracted from PlayPokerViewModel" changelog banner; dead `DismissEarnedToast` action removed |
| 2026-09-06 | features/room/impl/.../PlayPokerViewModel.kt | `CardBackStyle` FQNs → import; local `humanSeatIndex` in `handleHandEnded` shadowed the class property → `humanIdx`; dead `DismissEarnedToast` handler; 3 narrating init banners deleted |
| 2026-09-06 | features/room/impl/.../TableUiStateTest.kt | +1 test: heads-up blind badges asserted against the `BlindPosted` events `GameEngine.startHand` emitted; `activeFromSeats` takes a button index |
| 2026-09-06 | features/room/impl/.../PlayPokerViewModelTest.kt | Dropped `dismissEarnedToast_clearsList` — the only thing keeping a dead action looking alive |
| 2026-09-06 | docs/agent/ai-style-guide.md | +2 tips: mirror the engine's fallbacks when a projection re-derives its decision; don't let an FQN find/replace rewrite the `import` line |
