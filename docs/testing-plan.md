...
> - [x] **Bet/raise lines played to showdown over the wire** — the bug that started this (`9UQXGW`) was a raise line. *(✅ `WireBettingLinesTest.raise_ridesTheWire_isApplied_andPlaysToShowdown`: a full raise rides the wire, is reflected to both clients, the opponent calls, and the hand plays down to a five-card showdown with chip conservation.)*
> - [x] **River/turn fold over the wire** — fold after community cards are dealt. *(✅ `WireBettingLinesTest.foldOnALaterStreet_endsHand_andAwardsPotToTheOtherPlayer`: drives to the turn, folds, asserts the non-folder wins the accumulated pot and chips conserve.)*
...
> - [x] **Multi-hand: button rotation + stack carry-over** across `RequestNextHand`. *(✅ `MultiHandWireTest.acrossHands_buttonRotates_andStacksCarryOver`: hand 1 ends on a fold, the next hand deals with the button rotated and each seat's stack carried over (recovered via `stack + contributedThisHand`), not reset to the starting value. Companion `threeConsecutiveFullHands_dealThroughShowdown_chipsConserved` plays three consecutive full hands deal → preflop → flop → turn → river → showdown → next, asserting chip conservation and button rotation across each.)*
...
> - [x] **Action-pill ordering** — "Called 50" pills survive the same event/snapshot ordering as the winner fix (same `GameEventReceived` re-projection path). *(✅ `PlayPokerViewModelTest.actionTakenEvent_withoutFollowingSnapshot_rendersActionPill` (an ActionTaken with no following snapshot still paints the pill) + `streetAdvancedEvent_clearsStaleActionPills` (a street change wipes last-street pills, again with no snapshot).)*
> - [x] **Server-dealt table advances consecutive hands with no next-hand tap** — the B7/CARDS-25 stall: an Open/Public table is server-dealt, so once it's all-human nothing advanced it. *(✅ `MatchmakingPlayTest.serverDealtAllHumanTable_advancesConsecutiveHands_withNoNextHandTap`: an Open two-human table deals three consecutive hands driven only by acting each human's turn, never calling RequestNextHand. Unit-pinned by `ServerBotDriverTest.serverDealtAllHumanTable_autoAdvances_withoutNextHandTap`.)*
...
- [x] **`wireFormat_GameStateSnapshot_roundTripsLosslessly`** — server constructs a GameState, sends as JSON over WS, client decodes; deep-equality assert. (Pins the wire contract — any field added to one side without the other breaks this immediately.) *(✅ `RoomSocketWireFormatTest.gameStateSnapshot_roundTripsLosslessly` (`:libraries:rooms:impl`): a fully-populated `GameState` round-trips through the production `RoomSocketEventDto.serializer()` + `RoomSocketJson`. Pinned at the serialization seam, not the socket — the gameplay types are shared, so the live wire can't carry a field the round-trip drops.)*
- [x] **`wireFormat_allGameEvent_variants_roundTrip`** — same as above for every `GameEvent` sealed variant. *(✅ `RoomSocketWireFormatTest.everyGameEventVariant_roundTripsLosslessly` (all 7 `GameEvent` variants) + `everyPlayerActionVariant_roundTripsLosslessly` (all 6 `PlayerAction` kinds), enumerated so a new variant fails until appended.)*
...
- [x] **`twoClientsRace_sameIntent_serverDedupesViaNonce`** — the acting client resends the same intent under the same nonce twice at once; server processes once; both sends see `IntentAck.accepted` (server's idempotency contract). *(✅ `NonceRaceTest.twoClientsRace_sameIntent_serverDedupesViaNonce`: a duplicate-send race over the real socket — the session mutex serializes, the nonce dedup applies the fold once, both acks come back accepted, chips conserve. Realized as a same-client resend rather than two distinct users, because the server records a nonce only for an accepted intent so a second user's duplicate is a wrong-seat rejection, not a dedup — the same-actor resend is the real reconnect-resend contract.)*
...
- [x] **WS drops mid-`SubmitIntent`** — outbound frame may or may not have shipped; on reconnect, server's nonce dedupe should make the resubmit a no-op if processed, or fire normally if not. *(✅ `InFlightSubmitReconnectTest.submitThenDropAndReconnect_resendsSameNonce_isIdempotent`: the acting client folds (acked, hand completes), its real socket drops + auto-reconnects, then it resends the SAME nonce — the server's per-`GameSession` nonce ring (which survives the socket drop/reopen) short-circuits before turn validation and acks accepted on the already-Complete hand, so the fold never re-applies and chips conserve. The accepted ack on a finished hand IS the dedup proof — a fall-through to the engine would reject re-folding a completed hand. Combines the two halves previously pinned apart: `NonceRaceTest` (duplicate-nonce dedup) + `ChaosPlayTest` (drop/reconnect/resync).)*
...
- [x] **Client backgrounded > 30s during opponent's turn** — on resume, client's `connection` collector reconnects, `gameplayFrames` flow re-syncs, no stale state shown. *(✅ `ChaosPlayTest.idleClientBackgrounded_opponentActs_idleResumesAndResyncsToLiveState`: the non-acting client drops + blocks reconnects (sustained offline window, not a one-frame blip), the opponent folds and the server advances the hand to Complete while it's offline, then it unblocks — its real reconnect machinery + the socket's `replay = 1` latest-snapshot buffer converge it on the post-action stacks, no stale pre-background view.)*
- [x] **Two clients race the host role (host disconnects + reconnects fast)** — should not produce two effective hosts. *(✅ `SetupJourneyTest.hostDropsAndReconnectsFast_bothClientsAgreeOnExactlyOneHost`: the host drops (joiner is promoted), then reconnects before the grace reaper frees its seat; both real `LobbyViewModel`s reconverge on the original host and agree on a single effective host — never two.)*
...
- [x] **High-latency network (200ms+ RTT)** — `submit()` ack arrives slowly; UI doesn't double-submit on accidental tap. *(Built the guard, not just the test: `PlayPokerViewModel` now dedupes intent submission per decision point, keyed on the live state's `(handNumber, lastSequence)` — two taps before the resulting snapshot lands drop the second, so a slow ack / double-tap can't fire the same action twice. Cleared on a rejected submit so a corrected resubmit on the same turn still goes through. Pinned in `PlayPokerViewModelTest.submit_{doubleTapOnSameDecision_forwardsOnce,nextDecisionAfterSnapshot_forwardsAgain,rejectedThenRetriedOnSameDecision_forwardsAgain}`.)* The 200ms-RTT transport simulation itself remains an `:integration`-tier add.
- [x] **WS handshake during auth refresh** — the auth bootstrap was the source of one real iOS bug; pin the race. *(✅ `KtorRoomSocketTransportTest.open_suspendsUntilAuthReady_thenFiresHandshake`: starts the handshake while `awaitAuthReady()` is still pending (a gated `CompletableDeferred`), asserts the MockEngine sees zero requests, releases the gate, then asserts exactly one wss:// upgrade fires. Pins the order at the `KtorRoomSocketTransport` → `authedWebSocketSession` seam — the auth pre-flight runs before any TCP/TLS work touches the wire.)*
...
- [x] **Concurrent intents from multiple clients on the same hand** — server's per-session mutex serializes; both clients' acks arrive in some order. *(✅ `ConcurrentIntentTest.twoClientsActConcurrently_mutexSerializes_onlyTheActorMutates`: two real clients fire folds at the same live hand from real threads; the actor's applies, the non-actor's is rejected in either processing order, the hand completes and chips conserve. Wire-tier companion to the unit-tier mutex pin in `GameSessionTest`.)*
...
- [x] **`FakeRoomConnectionHandle` doesn't model `replay = 1` on `connection`.** A test for "late subscriber sees the last connection state" passes against the fake but the production handle has different semantics. Fix: back the fake with a real `MutableSharedFlow(replay = 1)` to match production. *(Already done in `RemotePokerSessionTest.FakeRoomConnectionHandle`: `_connection = MutableSharedFlow(replay = 1, extraBufferCapacity = 8)` + `_latestGameState = MutableStateFlow(null)` + `_gameplayFrames = MutableSharedFlow(replay = 0, extraBufferCapacity = 64)` mirror production's `ReconnectingRoomSocket` line-for-line. Late-subscriber coverage lives at the production-handle seam itself — `ReconnectingRoomSocketTest.gameplayState_replayReachesLateCollector` and `:apps:integration` `InHandPlayTest.gameplayCollectorAttachingAfterTheDeal_stillSeesTheTable`.)*
- [x] **`FakeRoomSocketTransport` doesn't simulate close → reopen on a single session.** Production WS can close and the next `open()` returns a fresh session. The fake handles this but the test contract isn't explicit. Add a `simulateReconnect()` helper. *(Landed: `FakeRoomSocketTransport.simulateReconnect()` bundles the latest-session `closeFromPeer()` + the next `primeSuccess()` so the close → reopen contract reads as one call; pinned by `ReconnectingRoomSocketTest.simulateReconnect_dropsCurrentSession_andLandsNextOpenOnFreshSession`.)*
