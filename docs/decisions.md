...
**Decision:** Ship the marker *in addition to* MetricKit, not instead of it. They answer different questions and neither replaces the other: `previous_exit` says how a day's runs ended and is authoritative about the *reason*; `app.previous_run` says where **this** run was standing when it stopped checking in, and is authoritative about nothing else. The one thing the marker buys that nothing else does is the force-quit split — swiping an app out of the iOS switcher backgrounds it first, so a force-quit lands as `background_exit` and a run that vanished believing it was on screen lands as `foreground_termination`.

ENG-25's objections were both correct and are handled rather than argued away. "Can't distinguish crash/ANR/OOM": true, so `foreground_termination` is documented as a candidate set to be read net of Sentry crashes for the same `previous_session_id`, never as a verdict. "Misfires on device restarts/upgrades": also true and unfixable, which is why the marker ships on **Android too** — Android has `ApplicationExitInfo` ground truth, so the two disagreeing there is the calibration that tells us whether the iOS reading can be trusted at all. An uncalibrated heuristic was the real problem with the 2026-07-11 version of this idea.

**Alternatives rejected:** iOS-only (matches the todo's framing and saves a `SharedPreferences.commit()` per transition, but leaves the heuristic permanently unvalidated — the calibration *is* the argument); persisting through DataStore like everything else in the app (an async store loses precisely the write that matters when the OS kills the process without warning); carrying the last route in the marker so the death site is on the event (redundant — `previous_session_id` joins to the dead run's own breadcrumbs in Loki, which is how CARDS-3 was reconstructed in the first place, and it keeps the marker cheap enough to write on a lifecycle callback).

## 2026-07-22 — Matchmaking auto-seats both searchers; the manual table chooser is gone (MP-35)

**Problem:** Two humans who searched at nearly the same time still didn't reliably end up playing. The first searcher's find opened a table and they sat waiting; the second searcher's initial candidates browse found tables and parked them on a manual "pick a table" chooser. Until that second person tapped Join, the server's auto-deal (proven by the chooser-join test) had nothing to deal — the exact stall the owner reported (CARDS-B0/AW). The single-affordable-candidate auto-join only covered the one-table case; two or more candidates always meant a chooser.

**Decision:** Option (a) from the deferred product decision — auto-seat, drop the chooser. The owner's own bug report is the product direction: "the moment there's enough people to play the game begins." On Start the client now calls `findTable` (the server's atomic find-or-create) immediately; the server picks the best table (most humans, oldest, affordable, MP-34 rescue) or opens a fresh one. `created = false` lands the searcher straight in the ROOM-11 pre-deal lobby (`SearchPhase.Joined`); `created = true` keeps the honest waiting radar with the ROOM-12 wait-time consolidation and the disclosed-bot fallback unchanged. `SearchPhase.Choosing`, the candidates chooser UI, `JoinCandidate`/`StartFreshTable`, and the chooser re-poll are removed; `/v1/matchmaking/candidates` stays, serving the wait-time consolidation poll. Funnel events: `matchmaking.candidates_shown` and `matchmaking.candidate_joined` are replaced by `matchmaking.matched` (find seated us with existing players).

**Alternatives rejected:** keeping the chooser and making the waiting side converge onto it (option b — preserves the "explicit buy-in accept" tap and the aspirational listing of unaffordable tables, but keeps a manual step between two willing players and a dealt hand, and the waiting-side convergence adds machinery to preserve friction); auto-joining the top candidate client-side while keeping the chooser for ties (leaves two code paths where the server already is the one atomic authority — the MP-34 decision's own argument).

**What's lost:** the aspirational disabled listing of unaffordable in-range tables (MP-36 kept them visible in the chooser). Affordability is still fully enforced — find only seats affordable tables and clamps a created table's buy-in — the tables just aren't browsable. If a browse surface is ever wanted again, the candidates endpoint still carries `affordable`/`minBalanceToSit` per table.

**Status:** Shipped. Red-first: `start_autoSeatsViaFind_evenWhenSeveralTablesExist` (VM parked on Choosing with find never called pre-fix). Existing pins kept green: matched-pair auto-deal (server + integration), ROOM-12 migration tiebreaks, bot fallback, seat_unaffordable terminal. QA MP-2 and the app-events wiki updated.

## 2026-07-21 — Public matchmaking is affordable by construction; four economy knobs are one invariant (MP-36)

**Problem:** Public "Find a table" games got stuck forever on "Dealing you in any moment now" even with real players seated. Root cause was an economy inconsistency, not a seating or convergence bug. Four constants that must agree didn't: the starter grant (10,000), the public-table entry bar (buy-in must be 25% of balance, so wallet covers 4 buy-ins), the matchmaker's tier-snap anchor (`BuyInTier.within` anchored on `RoomSettings.DEFAULT_BUY_IN` = 5,000), and the client's default search band (a fraction of balance, which produced 1,900-6,000 at a 10k grant). A 1,900-6,000 range snapped to a 5,000 table, which needs a 20,000 balance to clear the entry bar, so every fresh player failed `BelowEntryBar`, was silently dropped during funding, and the deal collapsed to a `Rejected` return that nothing logged. The table sat in Lobby with no error to anyone.

**Decision:** Keep the 4x entry bar (it is sound anti-ruin and anti-smurf, already documented in `docs/wiki/multiplayer.md`), and make the other knobs honor it rather than weakening the bar. Concretely:
- A single affordability predicate, `EntryBar.canSit(balance, buyIn)` (plus `minBalanceToSit` / `maxAffordableBuyIn`), is the one source of truth. The seat gate (`DefaultTableSessionService`), the matchmaker, and the test fake all call it, so they can never drift.
- `BuyInTier.ANCHOR = 1,000` is a dedicated matchmaker anchor, separate from `RoomSettings.DEFAULT_BUY_IN` (which is also the wire/host fallback and has a wide blast radius). `within` snaps toward the anchor, so a fresh player defaults to the 1,000 tier they can actually afford.
- The client default search band is fixed chips (500-2,000), not a fraction of balance. `BuyInTier` moved to `:libraries:gameplay` so the client's "what you'll get" preview and the server's snap use identical logic.
- Affordability is enforced where you *sit*, not where you *browse*: `find` and the auto-pick/rescue only ever seat an affordable table (and clamp a created table's buy-in to `maxAffordableBuyIn`), but the chooser still lists unaffordable in-range tables, flagged per-table (`affordable` / `minBalanceToSit`) so the client shows them disabled with a "Need X chips" label. The range you pick is "stakes I want to browse"; sitting is governed per-table. This keeps the ecosystem visible as aspirational without ever stranding a player.
- The silent failure is surfaced: the auto-deal is wrapped in a `matchmaking_auto_deal` span, its `Rejected` reason is logged and span-stamped, and a connected member who cannot be funded gets a terminal `seat_unaffordable` socket frame so the client shows "you need X chips" instead of an infinite spinner.
- A consistency-invariant test pins grant, anchor, band, and bar together (`EntryBar.canSit(STARTER_GRANT, BuyInTier.within(500, 2_000))`), so a future edit to any one knob fails loudly. This is deliberately a test, not a runtime config: these are correctness-coupled invariants, and a config that lets them drift independently would reintroduce this exact class of bug.

**Alternatives rejected:** lowering the entry-bar multiple (would let a fresh player commit their whole bankroll to one table, the same reasoning ROOM-13 already rejected when it separated `DEFAULT_HOST_BUY_IN`); repurposing `RoomSettings.DEFAULT_BUY_IN` as the anchor (too wide a blast radius); filtering unaffordable tables out of the chooser entirely (loses the aspirational "worth buying into" signal); moving the knobs into runtime config for V1 (they must stay mutually consistent, so a test-pinned invariant is safer than independent tuning).

**Status:** Shipped. Test-first: the reproduction (two fresh-grant searchers at the default band, stuck in Lobby pre-fix, now deals) is red on the old path and green on the fix. Server + client + gameplay + rooms suites, `assembleDebug`, and detekt green. Follow-ups filed in backlog: proactively peppering bots into public games, and a server-side matchmaking queue as the long-term convergence answer.
...
## 2026-08-20 — A stranded session is a typed 401 `account_not_found`, detected twice

**Problem:** A verified JWT naming a user id with no `auth.users` row (account deleted mid-session, or a token minted against a different Supabase project than the server's database belongs to) failed V11's `*_user_id_fk` on every per-user write. The generic `exception<Throwable>` handler turned that into a `500` whose body quoted the constraint, captured a Sentry issue per attempt, and told the client nothing it could act on — so the client kept syncing (AUTH-29).

**Decision:** One wire answer, two detectors. The answer is `401` with `error.code = "account_not_found"`. The detectors are (1) an explicit `SELECT 1 FROM auth.users` pre-flight in `PostgresProfileRepository.findOrCreateResult`, on the create branch only, raising `UnknownAuthUserException`; and (2) a `StatusPages` net that recognises a `23503` violation of any `*_user_id_fk` constraint and renders the same response.

The pre-flight alone would be incomplete — every repository that writes a per-user row would have to remember it — and the net alone would depend on parsing driver error output on the hot path. Together the net covers every table for free while the account-bootstrap path (`GET /v1/me`, the request that defines the whole session) gets a positive assertion. The pre-flight costs one index probe per account lifetime, because a `profiles` row is itself proof of an `auth.users` row: V11's FK plus its `ON DELETE CASCADE` means the two exist or vanish together.

**Why 401 and not 409:** the token isn't rejected — it verifies — so `409` is arguably the more literal status. But a client that doesn't recognise the code has to do *something*, and "re-authenticate" is the only useful something. A `409` would read as an unhandled error and keep the retry loop alive, which is the bug. `401` degrades safely; the code is what a current client routes on. The server sends no `WWW-Authenticate` header, so Ktor's bearer plugin doesn't spend a refresh round-trip on it.

**Client side:** the response feeds the existing `SessionRejectionBus`, the same seam a server-rejected token refresh uses, so the teardown, the `SessionExpired` state, and the recovery screen are all the ones already built and tested. Nothing new was added to the auth state machine. The knock-on is that `authedCall` sees the rejection epoch bump and remaps the failure to `AuthUnready(SessionExpired)`, which is logged at info — so the storm stops *and* stops filing error telemetry.

**Alternatives rejected:** checking `auth.users` in the JWT `validate` block (a DB round-trip on every authenticated request to catch a condition that essentially never happens); a new client-side bus or auth-repository entry point (`markSessionUnrecoverable` and the rejection bus already express exactly this, and a third path into the same teardown is a third path to keep correct).

## 2026-08-20 — A blocked client stops talking: latch the ban, break the circuit, stop reporting refusals

**Problem:** A banned account's client behaved exactly as it always had. Every sync went out, came back `403 {"reason":"banned"}`, and logged an error on the way back (Sentry CARDS-BG) — while the user sat on a blocking screen the whole time, so none of it could have changed anything. The `403` was a fire-and-forget signal on `AccessDeniedBus` with nothing holding it, which also meant nothing could observe the ban lifting: recovery was relaunch-only (ENG-35).

**Decision:** Three parts, one idea — the client should believe the server the first time.

**Latch it.** A new `BannedState` beside `AccessDeniedBus` remembers what the bus announces. `isBanned` is a plain synchronous read because the request interceptor consults it on every call. In memory, not persisted: a relaunch fires one doomed request before the first `403` re-latches it, which is one request rather than a stream, and a persisted ban the server later lifts would need its own invalidation story that the probe already provides for free.

**Break the circuit.** A Ktor client plugin, installed ahead of `Auth` so a blocked request is cut before the bearer plugin spends a token load on it, throws `BannedShortCircuit` for anything not on the allowlist. The allowlist is the recovery path and nothing else: `/v1/me` matched **exactly** (a prefix would let every `/v1/me/*/sync` write straight through, which is most of the traffic this exists to stop) and `/v1/app-config` (a blocked client we can't steer with a kill switch is worse than one extra request). `BannedShortCircuit` carries `ExpectedControlFlow`, so the telemetry sinks already drop it — the breaker working is not a failure.

**Let it lift.** A `200` from the exact probe path clears the latch, from the response validator. Gated on the path so an unrelated success can't unblock a client the server still refuses. Because nothing calls `/v1/me` on its own while the blocking screen is up, the screen now carries a "Check again" button behind a narrow `AccessStatusProbe` — contract next to `BannedState` (the allowlist is what makes the probe possible), implementation in the identity layer (which owns the `/v1/me` call). **Chosen over a background poll:** a poll would spend battery on a state that changes maybe once, and a user who has just appealed knows better than a timer when to look.

**Stop reporting refusals.** `SentryLogTree.shouldCaptureEvent` now also drops `401` and `403` via `isExpectedClientError()`, alongside the existing control-flow and offline exclusions. Deliberately narrow: `400`, `409`, and every 5xx still report, because those mean we sent something wrong or the server broke, which is exactly what the error panel is for.

**Alternatives rejected:** persisting the ban (adds a durable state whose only correct exit is the same probe, and gets a lifted ban wrong on the side that matters); putting the probe on `ProfileRepository` (would overload "refresh my profile" with "am I still blocked", and every fake would inherit a method it has no business having); a status-only telemetry filter that dropped all 4xx (hides real client bugs behind a moderation fix).

## 2026-08-20 — One classifier decides what counts as an error, and it lives in `:libraries:networking`

**Problem:** `ExpectedControlFlow`'s own KDoc promised that telemetry sinks drop marked throwables before they become error events. Exactly one sink honoured it. `SentryLogTree` filtered expected control flow and offline errors; `GrafanaLogTree` gated on log level alone and shipped the same `LogEntry` to Loki at full severity. 11 of the 13 prod client ERROR lines over 14 days were `AuthUnready` — 85% noise in the exact panel the nightly triage reads to find real bugs (ENG-44).

**Decision:** `Throwable.isExpectedFailure()` is the single predicate, and both trees consult it and nothing else. It folds the three things the codebase had already decided separately: the `ExpectedControlFlow` marker, `isOfflineError()` (ENG-34), and `isExpectedClientError()` (ENG-35).

**It lives in `:libraries:networking`, not `:libraries:core` where the item proposed it.** Two of the three predicates need Ktor's `ResponseException` and the platform socket types, and `:libraries:core` sits underneath both — putting the classifier there would mean either pulling Ktor into core or splitting the predicate across modules, which is the thing being fixed. `:libraries:telemetry:impl` and `:libraries:cards:impl` both already depend on networking, so no new edges. `ExpectedControlFlow`'s KDoc now points at the predicate, so the marker no longer reads as if marking alone were enough.
