...
- **Open discovery is range-overlap, not exact (clarified 2026-06-23).** Matching is range-based: the Find screen is a free-form range slider (no canonical snapping) and `findOrJoinPublic` matches `room.buyIn in minBuyIn..maxBuyIn`, so an Open table is found by any searcher whose range spans its buy-in. The handoff's "off-canonical tables barely match" was overstated — the only gap is a host at stakes outside common search bands (e.g. 3,000, below the default ~18k floor), which is inherent to range matching, not a bug. **Decided not to snap host-chosen stakes** to canonical tiers (it overrides a deliberate host choice the range slider already mostly accommodates). The genuine future improvement — a *present-matches-and-accept* step in Searching instead of silently auto-joining the first match — applies to Public too and is filed as a `[P3]` UX follow-up, not a blocker.
