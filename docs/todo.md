...
- `[P1]` **Wire `cards://auth/confirmed` deep-link mapping for `VerifyEmailRoute` — remaining piece.** URL scheme is registered on both platforms and `VerifyEmailScreen` already auto-refreshes on lifecycle resume (silent on still-pending so it never flashes a banner), so the same-device flow works whenever the screen is still on top when the browser bounces back. Remaining: add a `cards://auth/confirmed` `NavDeepLink` mapping in `OnboardingFeatureEntryPoint.screen<VerifyEmailRoute>` so the deep-link also lands the user *on* `VerifyEmailScreen` if they'd already navigated away (cold launch from the email tap). Open question worth the reviewer's call: `VerifyEmailRoute` currently takes `email: String` as a constructor arg, so the deep-link mapping either needs `cards://auth/confirmed?email={email}` (Supabase URL must include it) or `VerifyEmailRoute.email` becomes optional / falls back to the current session's email — pick one before implementing. **Out of scope:** the developer-todo entry for Supabase Site URL configuration (already tracked).
