...
- `[P2]` **Opponent level shows as 0 in MP.** The play screen renders every opponent at level 0: `RemotePokerSessionFactory.occupantsFor` hardcodes `level = 0`, and `TableUiState.badgeFor` returns `null` for remote humans (`// remote human in MP — level plumbing arrives later`). **Fix (mirror the avatar path):** add `xp: Long?` to the engine `Seat`; resolve each player's `totalXp` from `ProgressionRepository.find(userId)` at hand-start in `RoomSocketRoutes.handleStartHand` (same place `badgeProductIds` is resolved) → onto `SeatOccupant` → `Seat`; derive the level client-side via `levelProgressFor(seat.xp)` in `badgeFor`. **Decision needed:** level can change between hands — resolve fresh on `RequestNextHand` if we want it to tick up mid-session, or freeze-per-session like badges do today. Needs a server deploy. Broader "tapped-opponent full Player Card (badges + title + level)" lives in [backlog.md](./backlog.md). *(proposed 2026-06-18)*
...
- `[done]` **Late gameplay-subscriber drop ("stuck on dealing in") — fixed 2026-06-18.** A client whose `RemotePokerSession` subscribed *after* the host's `StartHand` (i.e. the non-host navigating to the play screen on the deal) missed the `GameStateSnapshot` because the client socket's gameplay flow was `replay = 0` — the table never rendered. Fixed by retaining the latest snapshot in a replay-1 surface (mirrors the server's `GameSession.state`); see [`ReconnectingRoomSocket.kt`](../libraries/rooms/impl/src/commonMain/kotlin/com/cards/libraries/rooms/impl/ReconnectingRoomSocket.kt). **Test-gap lesson:** every existing MP test attached *both* clients' gameplay collectors *before* `startHand`, so the production ordering (subscribe-after-action) was never exercised. Added a unit regression (`gameplayState_replayReachesLateCollector`) + an integration test that attaches a gameplay collector after the deal (`gameplayCollectorAttachingAfterTheDeal_stillSeesTheTable`).
- `[done]` **Opponent avatar (emoji + background color) on the seat — shipped 2026-06-18.** Was rendering initials because the avatar never crossed the wire. Now snapshotted from the profile at join → engine seat at hand-start → `GameStateSnapshot`, mirroring the `badgeProductIds` path. Covered by `seat_carriesOpponentAvatar_overTheWire`. (Equipped badges/titles on the opponent seat remain the broader V1.x item.)
- `[P1]` **Close the subscribe-after-action test-ordering gap broadly.** The two bugs above shared one root cause: tests subscribe before the action, the app subscribes after. Add a reusable harness scenario (e.g. `startHandBeforeJoinerMounts()`) and apply the **symmetry rule** — any hot `SharedFlow`/`StateFlow` a late subscriber depends on needs a "replay reaches late collector" test (the `connection` flow had one; the gameplay flow didn't). Pairs with Round 5 (chaos/ordering) below. *(proposed 2026-06-18)*
- `[P2]` **Extend `WireFormatContractTest` to assert each public seat field reaches an *opponent's* view** (avatar, badges, level, …), so a "we forgot to send X" silently-missing-field bug fails loudly. *(proposed 2026-06-18)*
- `[done]` **Winner not shown at showdown — fixed 2026-06-18.** The table re-projected only on GameState snapshots; `HandEnded` (a separate, unordered flow) just set a transient, so a snapshot-before-event ordering dropped the winner. Fixed by re-projecting on transient-changing events. Covered by `handEndedEvent_{after,before}CompleteSnapshot_rendersWinnerOnTable`.
- `[P1]` **Buff the *seams*, not the engine — the engine is already property-tested; every recent bug was at the wire + the VM projection.** Added (2026-06-18): a play-to-showdown integration test with end-to-end **chip-conservation** assertion (`DeeperPlayTest`), and VM winner-ordering regressions. **Still high-value to add:** (a) bet/raise lines played to showdown over the wire (not just passive call/check); (b) river/turn **fold** (only preflop fold was tested); (c) multi-hand button rotation + stack carry-over; (d) all-in / side-pot settlement *through the socket* (engine has it; the wire path doesn't); (e) reconnect-mid-hand that resyncs the winner; (f) a VM test that the action pills ("Called 50") survive the same event/snapshot ordering. **Principle:** for every "X over the wire" or "X projected in the VM" feature, write the test that subscribes/asserts in the *production order* (after the action), since that's where the bugs hide. *(proposed 2026-06-18)*
