...
- `[P2]` **Per-turn countdown — surface the timer on the table.** Server-side enforcement shipped: `TurnTimerDriver` auto-checks (when legal) or auto-folds a human who stalls past `RoomSettings.turnTimerSeconds`. The remaining gap is client UI — the active seat should show a visible, ticking countdown so an auto-action never lands without warning. No wire change needed: `GameState.settings.turnTimerSeconds` + `actingSeatIndex` are already on the client, so start a local countdown when `actingSeatIndex` changes (server stays authoritative on the actual fold). *(proposed 2026-05-30)*
  **Acceptance:** the active seat shows a visible countdown that resets each turn.
  **Hints:** the acting-seat affordance lives in the play-poker table (`PlayerArea.kt` / `OpponentsRow.kt`). **Out of scope:** per-player time banks / configurable clocks.
