...
### Telemetry / feedback
- **Optional contact-email field on Bug Report + Feedback screens.** Neither form has a way for the user to leave a callback email — when someone reports a real bug we have no way to follow up. Add an optional "Email (so we can follow up)" `TextField` to both screens, pre-fill from the claimed identity's email if the user has one, leave blank for anon users. Pipe the value into `AppTelemetry.captureUserFeedback` so Sentry's `UserFeedback.email` is populated alongside the message body. **Out of scope:** verifying the email or sending a confirmation; this is a callback channel for us, not a contact-form for them.
- **Wire `AppTelemetry.setUser(...)` on identity resolution.** `setUser(id, email, name)` exists ([AppTelemetry.kt:96](../libraries/cards/impl/src/commonMain/kotlin/com/cards/libraries/cards/impl/AppTelemetry.kt#L96)) and forwards to `Sentry.setUser`, but has no callers. Today every crash attributes to "unknown user." Wire it to fire on every `IdentityState.SignedIn` emission — including anonymous users (their `identity.userId` is stable per install, so we can still correlate session-to-session crashes pre-claim). Likely home for the call: `AppEventListener.onIdentityChanged` or a small dedicated observer in `:apps:compose`'s DI graph.
...
- **Collapse `ReviewLauncher` / `ReviewPrompter` into one interface.** Both ([ReviewLauncher.kt](../libraries/review/src/commonMain/kotlin/com/cards/libraries/review/ReviewLauncher.kt), [ReviewPrompter.kt](../libraries/cards/src/commonMain/kotlin/com/cards/libraries/ReviewPrompter.kt)) declare an identical `suspend fun requestReview()`. `AdaptedReviewLauncher` is a delegating proxy that adds zero behavior — the duplication exists because `:libraries:review` was added later for testability and nobody collapsed the older `ReviewPrompter` into it. Pick one (lean: keep `ReviewLauncher` since it lives alongside `ReviewPromptCoordinator`), move the platform impls to `:libraries:review:impl`, kill the adapter and `ReviewPrompter`. Cosmetic — do it the next time anything in this area opens up.
...
- **Stats screen achievement strip — sort by most-recently-earned.** The horizontal list of earned achievements on `StatsScreen` is unordered today. Sort by `earnedAt DESC` so the most recent unlock leads. Cheap.
...
- **Surface achievements earned during the hand on the showdown view.** When a hand triggers an achievement, the user sees the chips/XP land but the achievement itself only appears later as a notification or profile-side toast. The `HandResultSummary` already carries the `EarnedAchievement` list — render each as an inline card (title + one-line description) inside the showdown view between the XP/chip line and the "Next hand" CTA. This is the moment that earns its own celebration. Pairs with the ReviewPromptCoordinator's existing `AchievementUnlocked` trigger.
...
- **Orphaned room policy — robust, simple.**
  ...
  - **Treat >1 active rooms as recovery, not a normal state.** A user should only ever have one active room — anything else is the system failing to clean up after an app crash, force-quit, or network blip. When `getActiveRooms()` returns multiple, the client should post `leaveRoom` on all but the newest and keep just that one; the Home banner should be single-room. The DB should also only ever surface the newest. Server side needs to tighten the contract: WS heartbeat (Ktor has ping-pong built in) plus a sweep that hard-evicts after N missed pings rather than just marking `disconnectedAt`. **Open product call, needs human:** after the sweep eviction, do we *sit out* the user (auto-fold, keep the seat for a longer grace window) or *fully remove* them from the room? "Sit out" is friendlier; "remove" is cleaner. Pick before wiring the sweep behavior.
