...
- `[P1]` **Audit existing earnable pairings + redirect new ones toward volume / skill / placement.** Survey what's in [`EarnableCosmetics.kt`](../libraries/cards/src/commonMain/kotlin/com/cards/libraries/cards/EarnableCosmetics.kt) today (22 pairings; 8 are vanity titles, 6 of those 8 are single-showdown RNG); per pairing, decide keep / raise-the-bar / retire. New pairings come in three tiers based on cosmetic type:
  - **Earn-only** — badges, profile decorations, league-tier flexes, things it doesn't make sense to *buy*. Status signal: "this player put in the time / placed in the leagues."
  - **Earn-or-buy** (the wallet-rich-non-buyer sink) — cosmetic exists in shop at chip price *and* drops as an achievement unlock. Earned version carries the achievement badge alongside; bought version doesn't. Seed entries: the player heat-map widget (MP-only earn, medium-hard) and the win/loss-odds ratio display (harder earn, priced to bias the cohort toward purchasing).
...
- `[P1]` **Add `install_id` + L1 server-side orphan cleanup on /me.** Implements the V1 scope-cut design in [`recovery-and-orphaned-accounts.md`](./recovery-and-orphaned-accounts.md). Does **not** ship recovery_id, platform keychain, Welcome-back screen, or revival on reinstall — those are deferred to V1.x via [backlog.md](./backlog.md) Option B/C. Pieces:
  1. **Server schema:** new column `profiles.install_id UUID NULL` with an index; simple ALTER migration.
  2. **Client:** generate UUID on first launch if app-local storage doesn't have one (DataStore or simple file). Send `install_id` as a request header on every authenticated call.
  3. **/me handler:** read `install_id` from the request header; if it differs from `profiles.install_id`, UPDATE to the new value. Then enqueue the L1 cleanup task (background, doesn't block response).
  4. **L1 cleanup task:** SQL pre-filter `SELECT p.id FROM profiles p LEFT JOIN wallet_events we ON we.user_id = p.id AND we.source LIKE 'iap.%' WHERE p.install_id = :install_id AND p.id != :current_user AND p.is_anonymous = TRUE AND we.id IS NULL`. For each candidate, Kotlin verifies (still-anon, `level <= 1`, zero achievements, no recent wallet/room activity) and deletes via `SupabaseAdminClient.deleteUser` + `ProfileRepository.delete`. Verification stays in Kotlin so safety conditions can evolve without rewriting SQL.
  5. **Spec amendment:** update [product-spec.md §6.1](./product/product-spec.md#61-anonymous-by-default) — drop the "best-effort revival on reinstall" language; replace with "claim is the only durable identity path in V1."
  6. **Loss-disclosure UX:** thread "sign in to keep this" copy into the surfaces where the consequence of *not* claiming becomes visible — shop pre-purchase confirmation, stats banner, settings account section. Exact placements per the companion doc's "Loss-disclosure UX" section, with designer + product confirmation before they ship. **These are consequence disclosures, not the proactive claim prompts rejected by the [2026-05-20 decision](./decisions.md)** — different shape, different intent.
  **Files / hints:** signin sits in [`SupabaseAuthRepositoryImpl`](../libraries/identity/impl/src/commonMain/kotlin/com/cards/libraries/identity/impl/auth/SupabaseAuthRepositoryImpl.kt); /me handler in `MeRoutes.kt`; existing claim card lives in [`ProfileScreen.kt`](../features/profile/impl/src/commonMain/kotlin/com/cards/features/profile/impl/ProfileScreen.kt) as a starting point for the loss-disclosure copy. Sweep code that stays dormant: [`DefaultOrphanAnonymousSweep`](../apps/server/src/main/kotlin/com/cards/server/data/DefaultOrphanAnonymousSweep.kt) — don't delete the class yet; future cleanup commit rips it out. **Out of scope:** recovery_id, KMP keychain, Welcome-back screen, `/v1/recovery` endpoint, starter-grant dedup gate, L2 parked sweep — all in backlog as Option B/C upgrade paths.

- `[P1]` **Wire `cards://auth/confirmed` deep link so email verification auto-completes on resume.** Today Supabase confirms the email server-side at click-time but the user has to manually return to `VerifyEmailScreen` and tap "Check verification." With a deep link the browser hands control back to the app automatically and the screen's resume logic does the refresh. **Pattern:** deep link routes to the page; the page handles its own resume — no global side effects, generalises to password-reset later. Four mechanical pieces: (a) uncomment the `cards://` URL scheme in [`AndroidManifest.xml:45-55`](../apps/compose/src/androidMain/AndroidManifest.xml) and [`Info.plist:42-55`](../apps/ios/iosApp/Info.plist) (both currently template-commented), (b) add a `cards://auth/confirmed` deep-link mapping for `VerifyEmailRoute` in `OnboardingFeatureEntryPoint`, (c) add a lifecycle-aware effect in `VerifyEmailScreen` that calls `refreshSession()` on every foreground resume and navigates to home when `emailConfirmedAt` flips, (d) the developer-todo handles setting Supabase Site URL = `cards://auth/confirmed`. Keep the existing manual "Check verification" button as a fallback for cross-device flows (user confirms on laptop while phone is open). **Acceptance:** same-device flow — user signs up → opens email → taps link → browser opens → app comes back to foreground → `VerifyEmailScreen` auto-confirms and routes home; cross-device flow — manual button still works.
...
- `[P1]` **`room_sessions(state_jsonb)` snapshot table + write-through on every mutation.** New `room_sessions(session_id UUID PRIMARY KEY, state_jsonb JSONB, updated_at TIMESTAMPTZ)` overwritten inside the per-session mutex on every state change. Replaces the in-memory-only `StateFlow` as the source of truth. **Acceptance:** server restart mid-hand → registry hydrates from snapshot → players reconnect into the live game with their seats / stacks / current betting round intact. **Files / hints:** new migration; `SessionSnapshotWriter` analogous to the existing `PostgresGameEventWriter`; `GameSessionRegistry` hydrates on first read. **Out of scope:** the rolling event tail for animation-replay-on-reconnect (parked — see §B5). OTel traces cover the "every move debug" use case the event log was reaching for.

- `[P2]` **Retire the `game_events` write path.** Shipped 2026-05-28 against the prior event-sourced direction; the architecture flipped and nothing reads it. Cleanly remove `PostgresGameEventWriter` + its DI binding; drop the `game_events` table in a new migration. The `GameEventEnvelope` carrier may stay if it's still useful as an in-memory shape — reviewer call at retire time. **Out of scope:** the hybrid "rolling tail" option (parked in §B5).

### B1 — WS reconnect protocol

- `[P1]` **Snapshot-on-reconnect, no event tail.** On client reconnect (existing `RequestSnapshot` frame), respond with `Snapshot(state)` read from `room_sessions.state_jsonb`. Brief disconnect = client sees a single-frame state jump from before-disconnect to post-snapshot (animations don't replay). Acceptable jank for V1 given the 5-minute reconnect grace; revisit only if real users complain. **Files / hints:** `RoomSocketRoutes.kt` server-side; [`ReconnectingRoomSocket.kt`](../libraries/rooms/impl/src/commonMain/kotlin/com/cards/libraries/rooms/impl/ReconnectingRoomSocket.kt) client-side. **Out of scope:** event-tail catch-up (B5).
...
- `[P1]` **Wire OpenTelemetry traces on the server.** Add `io.opentelemetry.instrumentation.ktor` + the OTLP HTTP exporter to `apps/server`. **One trace per `SubmitIntent`**; spans for `validate-intent` → `engine-resolve` → `state-mutate` → `broadcast` → per-recipient `ws-send`. Attributes on every span: `session_id`, `user_id`, `hand_id`, `intent_type`. Default exporter target = stdout in dev (so traces show in `flyctl logs`); prod target = the Grafana Cloud OTLP endpoint resolved in [`developer-todo.md`](./developer-todo.md). Client-side OTel is deferred — V1 client errors flow through Sentry. **Acceptance:** a single `SubmitIntent` produces a connected span tree in dev stdout; in prod, the same tree is queryable in Grafana Cloud Tempo by `hand_id`. **Files / hints:** Ktor instrumentation + `GameSession.kt` is where the intent → resolve seam sits.
...
- `[P1]` **Wire OTel logs SDK on the server alongside traces.** Same OTLP exporter, same endpoint — adding `io.opentelemetry:opentelemetry-sdk-logs` + the Logback / SLF4J appender bridge ships server logs through the same pipe. The SDK auto-injects the active span's `trace_id` into every log record. **Why this matters:** in Grafana Cloud, clicking a span in Tempo opens the matching logs in Loki filtered by `trace_id` — the "I see a weird trace, show me every log line tied to it" flow. Without OTel logs the trace ↔ log correlation breaks and we're back to grep-and-reconstruct. **Acceptance:** a `logger.info("...")` call inside a span shows up in Grafana Cloud Loki with the parent `trace_id` populated; the Tempo span detail view's "Logs for this span" link returns the matching log line. **Files / hints:** server logging config in `apps/server`; bridge the existing Logback setup rather than swapping it.
