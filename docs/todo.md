...
LobbyViewModel `Leave`, HomeViewModel `Forfeit`, FeedbackViewModel `Submit`, BugReportViewModel `Submit`, AccountActionsViewModel `ConfirmSignOut`, and DeleteAccountViewModel `ConfirmDelete` now route their network calls through the existing `AppCoroutineScope` via `appScope.async { … }.await()`, so the server-side request outlives VM teardown. AGENTS.md → "SEAViewModel Pattern" documents the rule. Follow-up: audit additional candidates as they surface — end-of-session telemetry writes, EditProfile display-name/avatar writes (currently `viewModelScope`), any other "must reach the server" action whose UI doesn't care about the result.
