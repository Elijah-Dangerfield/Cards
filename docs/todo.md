...
- `[P2]` **More emote / blast-pack unlocks beyond the seeded Eliminator + Baller + Iron Stack + Convincer + Disciplined packs.** Five pairings shipped: `emotes_eliminator` for `BUST_DEALT_5` (V25), `emotes_baller` for `TRIPLE_UP` (V27), `emotes_iron_stack` for `NO_BUST_100` (V28), `emotes_convincer` for `WIN_BY_FOLD_10` (V29), `emotes_disciplined` for `GOOD_FOLD_25` (V30). Remaining work: extend the catalog — pick the next achievement → pack pairing each time a new themed bundle makes sense. Candidates worth considering: `BEAT_*_10` per-bot signature packs (one pack per personality) and a multi-suit theme tied to `SHOW_*` rare-hand achievements (`SHOW_STRAIGHT_FLUSH` / `SHOW_ROYAL_FLUSH`). **Out of scope:** the existing pathway itself.
