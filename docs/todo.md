...
- **Chip-themed bubble variant for poker chip dialogs.** Inside the poker screen, chip-related dialogs (rebuy, bust, chip rewards, soft-bust grant, tip-the-dealer) should use a circular gold bubble with a `$` glyph in the middle instead of the generic emoji bubble. New factory alongside `dialogEmoji(...)` — `dialogChipBubble(...)` — that paints a gold-filled circle + `$` and routes through the same chokepoint. Decide while building whether the gold treatment is a `surface` enum on the existing emoji bubble or a sibling primitive (lean: sibling — the $ isn't an emoji glyph and shouldn't share the unicode rendering path).
- **Dialog emoji bubble: bottom corners render as a straight line.** With an emoji bubble present, the dialog's bottom corners stop looking rounded. Likely an interaction between the `BodyGap` push-down in [EmojiBubble.kt:124](../libraries/ui/src/commonMain/kotlin/com/cards/libraries/ui/components/dialog/EmojiBubble.kt#L124) and the static notched shape in [NotchedSheetShape.kt:60](../libraries/ui/src/commonMain/kotlin/com/cards/libraries/ui/components/dialog/bottomsheet/NotchedSheetShape.kt#L60) — the shape carves the top notch but its bottom-corner path doesn't follow the content bounds once the column grows past a threshold. Reproducible on any emoji-bubble dialog. **Files:** [Dialog.kt:111](../libraries/ui/src/commonMain/kotlin/com/cards/libraries/ui/components/dialog/Dialog.kt#L111), `NotchedSheetShape`.
...
- **`unlock_only` flag on products + Trophy Case surface.** [product-spec.md §4.2](./product/product-spec.md#42-the-unlock-only-catalog) is structurally load-bearing: legendary achievement cosmetics, league-tier cosmetics, RFT cosmetics, achievement-chain cosmetics are **never in the shop, ever.** The shop catalog and the unlock catalog must be *disjoint sets.* Server side landed 2026-05-21: `unlock_only BOOLEAN NOT NULL DEFAULT FALSE` column on `products` (migration V10), `ProductsTable.unlockOnly` Exposed column, and `PostgresProductCatalogSource.read` filters `WHERE unlock_only = FALSE` so the shop never surfaces them. Remaining work:
   ...
   - **V1-blocker** for any prestige cosmetic. Acceptable to ship V1 if the unlock-only catalog is *empty for V1* and Trophy Case is deferred — but that's a content decision. The filter is in place either way; V1 can ship with zero unlock-only rows and the shop is unaffected.
...
- **Equipped item exists while inventory is empty.** Repro 2026-05-21: red felt shows as equipped, but My Items is empty. Equipment and Inventory were independent DAOs with independent sync bootstrappers — nothing enforced "equipped ⇒ owned." Server side landed 2026-05-21: `POST /v1/inventory/sync` returns `owned: List<OwnedItemDto>` alongside the per-purchase results. Client side landed 2026-05-21: `InventoryRepository.applyServerSnapshot(...)` writes server-authoritative rows as Confirmed, drops local Confirmed rows that the server no longer agrees the user owns, and leaves in-flight Pending rows alone (parallel to `EquipmentRepository.applyServerSnapshot`). `InventorySyncServiceImpl` now always POSTs (even with empty pending) so it folds the snapshot back in on cold launch / device-switch. Remaining work: equipment consistency invariant — after equipment + inventory sync resolve, drop any equipment row whose productId isn't in the authoritative inventory, and log the drop so we can see how often it fires in the wild. The hook likely lives in `EquipmentSyncBootstrapper` (after observing inventory) or as a new `EquipmentRepository.dropOrphanEquipment(ownedProductIds)` method called from `InventorySyncServiceImpl` after `applyServerSnapshot`. **Files:** [EquipmentRepositoryImpl.kt](../libraries/cards/impl/src/commonMain/kotlin/com/cards/libraries/cards/impl/EquipmentRepositoryImpl.kt), [InventorySyncServiceImpl.kt](../libraries/cards/impl/src/commonMain/kotlin/com/cards/libraries/cards/impl/InventorySyncServiceImpl.kt).
- 
