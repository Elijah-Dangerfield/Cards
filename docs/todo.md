...
- `[P1]` **Audit remaining hot flows for the late-subscriber symmetry rule.** The reusable harness scenario (`startHandBeforeJoinerMounts()` in `PlayPokerViewModelMultiplayerIntegrationTest`) + the gameplay-flow replay coverage shipped — `FakeRoomConnectionHandle` now mirrors production's replay-1 latest-snapshot merge, and a VM-level late-mount test proves a play screen mounting after the deal replays the live table. Remaining: sweep the other MP hot `SharedFlow`/`StateFlow`s a late subscriber depends on and add a "replay reaches late collector" test for any still missing one. Pairs with Round 5 (chaos/ordering) below. *(proposed 2026-06-18)*
