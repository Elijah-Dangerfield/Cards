...
- **Previews on cosmetics in the *shop*.** My Items now renders felts / card backs via [`CosmeticPreview`](../libraries/ui/src/commonMain/kotlin/com/cards/libraries/ui/components/poker/CosmeticPreview.kt). The shop grid tile + purchase-confirm sheet still show only the emoji — pulling `CosmeticPreview` into those surfaces is the same primitive, just deciding *where* on the sheet/tile it sits and at what size. Bigger visual call than the My Items 48dp row; size + placement want a design judgment before landing. **Files / hints:** [PurchaseConfirmSheet.kt](../features/shop/impl/src/commonMain/kotlin/com/cards/features/shop/impl/PurchaseConfirmSheet.kt), [ShopScreen.kt](../features/shop/impl/src/commonMain/kotlin/com/cards/features/shop/impl/ShopScreen.kt) (`ProductIcon` is the tile renderer at line ~571).
