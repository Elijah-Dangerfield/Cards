...
- **Equipped item exists while inventory is empty.** Repro 2026-05-21: red felt shows as equipped, but My Items is empty. Equipment and Inventory are independent DAOs with independent sync bootstrappers ([EquipmentRepositoryImpl.kt](../libraries/cards/impl/src/commonMain/kotlin/com/cards/libraries/cards/impl/EquipmentRepositoryImpl.kt), [InventoryRepositoryImpl.kt](../libraries/cards/impl/src/commonMain/kotlin/com/cards/libraries/cards/impl/InventoryRepositoryImpl.kt), and the two `*SyncBootstrapper`s next to them) — nothing enforces "equipped ⇒ owned." Server-side foundation landed 2026-05-21: `POST /v1/inventory/sync` now returns `owned: List<OwnedItemDto>` alongside the per-purchase results (mirrors `POST /v1/equipment/sync` which has always returned the full snapshot). Backwards-compatible additive field. Remaining work is purely client-side: (1) `InventorySyncServiceImpl` consumes the `owned` list and hands an authoritative snapshot to `InventoryRepository.applyServerSnapshot(...)` (new method on the repo + DAO, parallel to `EquipmentRepository.applyServerSnapshot`); (2) once inventory is server-authoritative, add the equipment consistency invariant — after equipment sync resolves, drop any equipment row whose productId isn't in the synced inventory, and log the drop so we can see how often it fires in the wild. **Files:** the two bootstrappers + repos above; the post-sync reconciliation likely belongs in `EquipmentSyncBootstrapper` after it observes inventory.
- 
