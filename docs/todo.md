...
- `[P2]` **More emote / blast-pack unlocks beyond the seeded Eliminator + Baller + Iron Stack + Convincer + Disciplined + Grinder packs.** Six pairings shipped: `emotes_eliminator` for `BUST_DEALT_5` (V25), `emotes_baller` for `TRIPLE_UP` (V27), `emotes_iron_stack` for `NO_BUST_100` (V28), `emotes_convincer` for `WIN_BY_FOLD_10` (V29), `emotes_disciplined` for `GOOD_FOLD_25` (V30), `emotes_grinder` for `HANDS_1000` (V32). Remaining work: extend the catalog — pick the next achievement → pack pairing each time a new themed bundle makes sense. Candidates worth considering: `BEAT_*_10` per-bot signature packs (one pack per personality — needs five new packs shipped together to avoid an inconsistent half-rollout), `CHALLENGING_10_WINS` (toughness-tier theme — risk of emoji collisions with `emotes_fierce`), `DOUBLE_UP` (mid-tier doubling — pairs nicely with a "Doubler" pack if emoji overlap with the achievement's own 📊 icon can be avoided), or `REACH_LEVEL_25` (level capstone — natural fit for a card back rather than an emote pack). **Out of scope:** the existing pathway itself.
