...
- **ENG-20 `[P1]` Rebuild sync triggers on the level-based `runWhen` primitive (approved plan — follow it).** Problem: `UserScopedSyncCoordinator` only syncs when it catches a `UserChanged` event from a shared bus that replays a single event. Any later boot event (like the cold-boot foreground) evicts that replay slot, so a fast app launch can miss the sign-in trigger entirely and never sync. The coordinator also ignores connectivity returning, and a sync blocked because auth wasn't ready yet is never retried. This fragility is the prime suspect for why the owner's 07-09 relaunch never uploaded its pending chips.
  **Acceptance:** implement the approved plan in [`docs/plans/eng-20-runwhen-triggers.md`](plans/eng-20-runwhen-triggers.md) exactly as written — phases 1 and 2: a generic `runWhen(key, refireOn, retry)` primitive in `libraries/flowroutines`, `SyncTriggers` wiring in `libraries/cards/impl`, the coordinator rewritten as one `runWhen` per syncer, PlayStyle/InAppMessage migrated off their hand-rolled event triads, `AppEvent.AccountClaimed` deleted end to end, and the plan's full two-suite test list (including the prod-incident boot-race regression) green.
  **Hints:** the plan supersedes earlier hints — notably auth stays a replay-1 SharedFlow (no `StateFlow` conversion; the plan explains why). The six `UserScopedSyncer` repos don't change. Ranked failure modes in case `docs/agent/feedback-cases/2026-07-09-chips-vanish-on-restart.md`.
