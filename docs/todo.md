...
- `[P1]` **Add `FIRST_BUST_DEALT_MP` / `BUST_DEALT_5_MP` siblings and wire the server grant route to refuse `AchievementMode.MULTIPLAYER` ids.** The `AchievementMode.{EITHER, BOTS, MULTIPLAYER}` enum + per-achievement `mode` field already landed on [`Achievement.kt:117`](../libraries/cards/src/commonMain/kotlin/com/cards/libraries/cards/Achievement.kt) and the bot-mastery / difficulty / busts-dealt ids are tagged `BOTS`; what's missing is the MP side. **Acceptance:** new ids `FIRST_BUST_DEALT_MP` / `BUST_DEALT_5_MP` land with `AchievementMode.MULTIPLAYER`, registry entry, counter stub returning 0 (with a TODO referencing Phase 4.2); the client `POST /v1/me/grants/achievement/{id}` path refuses any `MULTIPLAYER` id (returns 403) by adding those ids into the `serverWitnessed` set on [`ClientGrantableAchievements.Default`](../apps/server/src/main/kotlin/com/cards/server/domain/ClientGrantableAchievements.kt) (which is empty today); tests pin the registry shape + the 403 path. **Out of scope:** the actual server-witnessed grant seam (Phase 4.2); brand-new achievement ids beyond the two siblings.
