...
- `[P2]` **Explain the "Practice tier · bots present" label — tappable + auto-shown at game start.** The label (`PracticeTierLabel` in [`PlayPokerScreen.kt`](../features/room/impl/src/commonMain/kotlin/com/cards/features/room/impl/PlayPokerScreen.kt)) is a non-clickable pill today, and players don't know what it costs them. What it actually means: when bots are present **and** humans don't equal-or-outnumber them (`MultiplayerCredit.qualifies` = false — so 1 human + bots, or bots outnumbering humans, *including a private game where the host added bots*), the hand is credited as **practice**: XP is **halved (0.5×)** and **multiplayer-only achievements are blocked** (`HandResultSummaryBuilder` downgrades `XpMode.MULTIPLAYER → BOTS`; `XpCalculator` `BOT_MULTIPLIER = 0.5`; `AchievementRepositoryImpl.modeAllows`). When the buy-in wallet economy lands ([B3 plan](./plans/mp-chip-buyin-economy.md)), the same `qualifies` gate is the natural switch for whether **real chips settle** too. Make the pill tappable → a dialog that explains this in plain language, and **auto-open it once at game start** so the player knows before they play. *(found 2026-06-21 playtest; the user assumed it means "no chips earned" — close: today it's halved XP + no MP achievements, and MP chips aren't settled for anyone yet.)*
  **Hints:** wrap `PracticeTierLabel` in a `clickable`; reuse the standard `Dialog`; auto-show keyed once per game (e.g. first state for a room code), don't re-pop each hand. Threshold logic is in `MultiplayerCredit.qualifies`.
...
- `[P2]` **Make card backs & felts much cheaper.** These are **self-only** cosmetics — nobody else at the table sees your card back, and the felt is low-impact — so their current prices are too steep for the value: card backs run **4,000–15,000** chips (e.g. `cardback_marble` 4000, `cardback_skulls`/`cardback_fire` 5000, `cardback_holographic` 15000) and felts **1,500–4,000** (`felt_royal_red`/`felt_midnight_blue`/`felt_pine_green` 1500, `felt_charcoal` 2000). Drop them all to genuinely cheap (rough target: card backs ~500–1,500, felts ~250–750; keep relative tiering so the fancy ones still cost a bit more). Prices live in the server `products` table (`cost_chips`), seeded in `apps/server/src/main/resources/db/migration/V5__products.sql` (+ V20/V23/V24 for card backs, V15/V16 for felts). Migrations are append-only, so add a new `Vxx__lower_cosmetic_prices.sql` with `UPDATE products SET cost_chips = … WHERE …` — don't edit old migrations. The shop catalog (`GET /v1/products`) reflects it on next deploy (5-min cache). *(found 2026-06-21 — owner's call: these aren't desirable enough to be premium-priced)*
