...
**Last reviewed:** 2026-06-24 · **Companion to:** [product/product-spec.md](./product/product-spec.md), [backlog.md](./backlog.md), [developer-todo.md](./developer-todo.md)
...
- `[P2]` **Graduate lifetime hand + achievement-progress counters to the server.** The `progression` hand counters (handsPlayed/won/folded/lostAtShowdown/botHandsPlayed) and the achievement *progress counters* (no-bust streak, per-bot wins, …) are client-local — they zero on account switch / reinstall, so a switched-in account shows correct XP/level + earned badges but zeroed hand counts. Carry them in their respective syncs (decided to lift, not accept-reset — `decisions.md` 2026-06-15). The hand counters double as the server `hands_finished` the MP-achievement floor wants. *(proposed 2026-06-14)*
...
- `[P2]` **Tap-an-opponent sheet — "view full profile" tap-through.** The human-variant "Add friend" affordance shipped (a Friend section on the player card sends a request, optimistic Sent flip with un-flip on server reject). Remaining: a "view full profile" tap-through, gated on profile-of-a-stranger being a real route (doesn't exist yet). *(This sheet is the at-table **Player Card** surface — see the Player Card feature below + `decisions.md` 2026-06-06.)*
...
- `[P1]` **Home active-room banner — live WS-pushed source.** The banner refreshes only on foreground / connectivity-regained, so a host closing the room (or a server-side forfeit/grace expiry) elsewhere won't reflect while Home is open until the next foreground edge. Hang the flow off a durable WS/presence push so it updates live.
...
- `[P3 — gate on leagues/leaderboards]` **Server-derive level-up grants instead of trusting the client.** The server applies whatever `levelup_<level>` chip delta the client sends (only guard: no-below-zero) and gates the cosmetic grant by product allowlist, not by *level reached* — so a tampered client can mint level rewards it didn't earn. Harmless for play-money, but the trigger to fix is when XP/chip totals feed leagues/leaderboards. Fix: on progression-sync the server derives level from its reconciled `total_xp` against the curve it already serves, grants `levelup_<level>` itself (idempotent), and caps client-claimed amounts.
  **Hints:** port the `levelProgressFor` interpreter from [`Level.kt`](../libraries/cards/src/commonMain/kotlin/com/cards/libraries/cards/Level.kt) into `:apps:server` (don't extract a shared module — see `decisions.md` 2026-06-21); grant precedents are `PostgresWalletRepository.apply` + `LevelGrantableProducts` / `GrantsRoutes`; client path is `LevelUpRewardGranter`.
...
- `[P1]` **Online-presence signal + real online-friends list on Home.** Server emits presence on WS connect/disconnect + stores last-seen / current-room; client subscribes once per session, filtered to friend ids. [`FriendsStrip.kt`](../features/home/impl/src/commonMain/kotlin/com/cards/features/home/impl/FriendsStrip.kt) already takes `List<FriendOnline>`; today the Home `onlineFriends` list is still the fake `defaultOnlineFriends()` in `HomeScreenContent` (the pending-requests badge is now real). Swap the fake list for the presence-filtered friend set once the signal lands.

- `[P1]` **Banned-account enforcement — client half (route the `403` to `BlockingErrorScreen`).** Server gate shipped: every authenticated route now returns `403 {reason, until, appealUrl}` for a banned caller (native `auth.users.banned_until`, `ModerationRepository` + the auth validate→challenge path; see `decisions.md` 2026-06-24). The client doesn't parse it yet — a banned user's calls 403, the token-refresh classifier reads that as `SessionExpired`, and they see the generic expiry screen, not a ban screen. Build the client half: parse the `403` `AccessDeniedResponse` envelope in the `NetworkClient` failure path, signal a typed "access denied" state (not `SessionExpired`), and route to `BlockingErrorScreen` with localized copy keyed off `reason` (copy lives in `:libraries:resources`) + the `appealUrl` action.
  **Acceptance:** a banned account sees `BlockingErrorScreen` with ban copy + appeal link (not the session-expiry screen); a clean account is unaffected.
  **Hints:** server contract is `AccessDeniedResponse` in [`AccessControl.kt`](../apps/server/src/main/kotlin/com/cards/server/plugins/AccessControl.kt); client seams are `RefreshFailureClassifier` / `SessionRejectionBus` / `NetworkClientImpl` and `BlockingErrorScreen` in `:libraries:navigation:impl`. **Out of scope (server, deferred):** suspended-vs-banned split (needs an app-level moderation table for a reason + per-user appeal URL; the native flag only carries banned-until), auto-ban triggers, shadow-bans, the review dashboard.
...
- `[P1]` **Stand up detekt as the project's custom-rule framework, gated in CI + pre-push.** The point is a growable set of AGENTS.md conventions the build mechanically enforces — both in CI and on `.githooks/pre-push` — so neither humans nor the nightly agents can violate them. Land the framework + the first rule now; the rest are cheap follow-ons. *(proposed 2026-05-30)*
  ...
  **Hints:** convention plugins live in `build-logic/`; existing `.githooks/` has `commit-msg`. **Out of scope:** migrating the existing string violations (`AppGuardLayer.kt`, …) — separate cleanup once the gate exists.
