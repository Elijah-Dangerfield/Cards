...
- `[P1]` **Land the registry split for bot-only vs MP-only achievements + add the `FIRST_BUST_DEALT_MP` / `BUST_DEALT_5_MP` siblings.** The two tier-blind re-anchors (DONT_CALL_IT_COMEBACK → ≤10 BB / ≥100 BB; POT_5000 → ≥25× BB) landed but they're mode-agnostic today. **Acceptance:** every achievement in `AllAchievements` carries an `AchievementMode` (BOTS_ONLY / MP_ONLY / ANY); the client `POST /v1/me/grants/achievement/{id}` path refuses to grant `MP_ONLY` ids (returns 403) so the future server-witnessed flow has a single source of truth; new MP-keyed siblings `FIRST_BUST_DEALT_MP` / `BUST_DEALT_5_MP` land with `AchievementMode.MP_ONLY`, registry entry, counter stub returning 0 (with a TODO referencing Phase 4.2), and tests pinning the registry shape. **Files / hints:** registry + `AchievementMode` live in `:libraries:cards`; [`ClientGrantableAchievements.kt`](../apps/server/src/main/kotlin/com/cards/server/domain/ClientGrantableAchievements.kt) on the server already has a `serverWitnessed` set — MP-only ids go there. **Out of scope:** the actual server-witnessed grant seam (Phase 4.2); brand-new achievement ids beyond the two siblings.
