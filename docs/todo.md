...
LobbyViewModel `Leave`, HomeViewModel `Forfeit`, FeedbackViewModel `Submit`, BugReportViewModel `Submit`, and AccountActionsViewModel `ConfirmSignOut` now route their network calls through the existing `AppCoroutineScope` via `appScope.async { … }.await()`, so the server-side request outlives VM teardown. AGENTS.md → "SEAViewModel Pattern" documents the rule. Follow-up: audit additional candidates as they surface — delete-account, end-of-session telemetry writes, any other "must reach the server" action whose UI doesn't care about the result.
