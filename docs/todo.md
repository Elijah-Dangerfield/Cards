...
- `[P1]` **Buff the *seams*, not the engine — wire + VM-projection test fan-out.** The engine is property-tested; every recent MP bug lived at the wire or the VM projection. Add, each in *production order* (subscribe/assert after the action): (a) bet/raise lines played to showdown over the wire (not just passive call/check); (b) river/turn **fold** (only preflop fold is tested); (c) multi-hand button rotation + stack carry-over; (d) all-in / side-pot settlement *through the socket*; (e) a VM test that the action pills ("Called 50") survive event/snapshot ordering. *(proposed 2026-06-18)*
