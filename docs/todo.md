...
- `[P2]` **Move the level ladder + reward table to app-config (server-tunable); grant by key, reconcile against config thresholds.** Today the XP-per-level curve (`Level.kt`, hardcoded `N²×100`) and the reward table (`LevelRewardTable` in `LevelReward.kt`) are compile-time client constants — retuning pacing or prizes needs a release. Lift both into the app-config tree (`progression.*`) so they ship a bundled default but update server-side without a build. Each reward carries a stable **grant key** (`levelup.10.chips`, `boost.xp_2x`); the client keeps granting optimistically + offline keyed by it (idempotent, as today). The server already holds authoritative `total_xp`, so it **reconciles** the claimed grants in the progression-sync response (confirm/void) — no per-hand replay, since validating "reached level N" is a threshold compare (`total_xp` against level N's start-XP from the same config), not a gameplay replay. See [`decisions.md`](./decisions.md) 2026-06-17. *(proposed 2026-06-17)*
  **Depends on:** the `JsonConfigValue` config-lib extension (Remote config section) for the structured ladder payload. **Pairs with:** the cosmetic reward kind (above) for the full reward set.
...
- `[P2]` **`JsonConfigValue<T>` — a structured (non-scalar) configured value.** The config system resolves flat dotted-path **scalars** today (`FlagConfigValue` / `IntConfigValue` / `StringConfigValue` — each a multibound class with a `path` + `default`). Variable-length structured data (the level ladder + reward table, future loot tables) can't be flattened into fixed scalar paths. Add a typed base that resolves **one** path to a JSON payload and deserializes it to a Kotlin model with a bundled default (decode failure → default, so a malformed server value never bricks the client). Use it alongside the grouped-interface convention (`IdentityConfig` + impl): a `ProgressionConfig` interface whose impl reads scalar knobs by path and the ladder via the JSON value. **Note:** a JSON blob won't round-trip through the per-key QA dashboard toggles — acceptable, the ladder is tuned server-side, not per-session. *(proposed 2026-06-17)*
