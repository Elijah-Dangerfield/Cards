...
- `[P2]` **"First multiplayer game" achievement.** Earn an achievement the first time a player finishes a multiplayer hand — a welcoming first-milestone before the `HANDS_100_MP` grind. Count-based server-witnessed id, so it rides the path that already ships: add a new id (e.g. `FIRST_HAND_MP`) at threshold `1` in `DefaultServerWitnessedAchievements.COUNT_THRESHOLDS` and define the achievement on the client (title/copy/rarity). **Record-only — no reward prize** (no cosmetic, no chips); the badge itself is the reward. No new server signal needed — the finished-hand count already drives it.
  **Acceptance:** finishing one multiplayer hand earns + surfaces the achievement; re-evaluation stays idempotent.
  **Hints:** mirror the `HANDS_100_MP` wiring, minus the reward grant.
...
- `[P2]` **Tighten the orphan-anon account sweep — never delete real progress.** Deletion of an abandoned anonymous account (today: `DefaultOrphanInstallSweep`, fired on `GET /v1/me` when the same device/install id is now bound to a *different* active anon account) must additionally skip any account above **level 1** — a high-XP account is preserved, never deleted, even when superseded on its device — on top of the existing no-real-money-purchase / no-earned-inventory / not-in-a-live-room gates. The bias is to leak orphan rows rather than ever delete someone's progress by accident. *(the ≥1-year-inactivity deletion branch is deferred — see [post-launch.md](./post-launch.md))*
  **Acceptance:** an anon account with a purchase or above level 1 is never deleted by the sweep; a truly-untouched superseded anon account still is.
  **Hints:** `DefaultOrphanInstallSweep.kt` — add the XP guard to the existing per-candidate checks. Rationale in [`decisions.md`](./decisions.md) 2026-06-19.
...
- `[P2]` **Buff the seams — multi-player side-pot settlement over the wire.** A *side pot* needs 3+ seats with unequal stacks — a short stack all-in plus two larger callers — but the harness only seats two. Add a 3-client table (`seatThreeAndConnect`) + a multi-hand setup that depletes one stack so the next all-in produces a genuine side pot, then assert each pot settles to the right eligible seats through the wire. The engine's side-pot math is property-tested; the gap is the end-to-end wire path.
