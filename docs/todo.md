...
- **Seed starter inventory atomically inside profile creation (replaces the per-/v1/me felt_default grant).** **Problem:** today every /v1/me hit calls `inventory.recordEarnedGrant(felt_default)` so new users land on My Items with at least one row. The grant is idempotent, but the read happens every request, and the grant pathway was built for *earned* items (achievements). Reusing it for "starter equipment" blurs the contract. New users should also start with the default blue card back and a starter emoji-blast pack, and adding each through the grant pathway compounds the per-request cost. **Approach:** move starter-kit seeding into `ProfileRepository.findOrCreate` so it runs exactly once, atomic with the profile insert. Starter kit is a fixed productId list (today: `felt_default`, `cardback_default`, `emoji_pack_starter`) written in the same transaction. Drop the felt_default grant from `MeRoutes.kt`. Achievement-grant endpoint stays untouched. **Acceptance:** new user → profile + three default inventory rows committed in one transaction; `MeRoutes.kt` no longer calls `recordEarnedGrant` for defaults; catalog has `cardback_default` (paired with the existing `CardBackStyle.Default` blue) and `emoji_pack_starter`, both `unlock_only = TRUE`; existing users get backfilled (one-shot migration); `EmptyInventory` test stub in `MeRoutesTest.kt` reverts to throwing on `recordEarnedGrant`. **Files / hints:** `apps/server/.../ProfileRepository.kt` + `PostgresProfileRepository.kt` (extend `findOrCreate` transaction); `apps/server/.../routes/MeRoutes.kt` (remove felt_default grant block); `apps/server/.../db/migration/V18__starter_inventory.sql` (catalog rows + backfill). Client renderer already returns `CardBackStyle.Default` blue for unknown card-back ids, so no client change for the card-back visual — emoji pack needs a real product row. **Out of scope:** changing the achievement-grant pathway (separate bullet); decoupling ProfileRepository's new inventory import (see `docs/backlog.md` → "ProfileRepository → InventoryRepository import").
