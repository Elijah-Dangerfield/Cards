...
- **Audit text sizes across the app — RankDetailSheet aligned with StatsScreen 2026-05-22; broader sweep still open.** RankDetailSheet's body copy (RankHero subtitle, HowRankWorks trailer, ClaimAccountCard paragraph, Bullet text) bumped from `Body.B400` → `Body.B500` to match StatsScreen's `Body.B500` body convention; the bottom-of-screen Bullets ("Where you'll see it") no longer read meaningfully smaller than the rest of the UI. StatsScreen continues to use `Body.B400` only for genuine *captions* (XP-to-next-level subline under the hero, StatTile labels, EventRow sublines) — that's the intended pattern: B500 is body, B400 is captions, callers shouldn't reach for B400 to make body copy "lighter looking." Remaining sweep: walk the other feature screens (room/shop/profile/home) and confirm any `Body.B400` is genuinely captional, not body. Pairs with the next bullet — if 90% of body callsites have to spell out `typography = Body.B500`, the default (`Body.B600`) is the lever, not per-callsite overrides.
- **DS-first text component default.** Whatever `Text` resolves to when called without a typography argument should match what a screen wants in 95% of cases. Today the default is `Body.B600` (set in `DefaultTypography.Default`); in practice screens almost always override to `Body.B500`. If the broader audit confirms this pattern, the right move is to flip `DefaultTypography.Default` to `Body.B500` so screens stop needing the explicit override — and the few cases that genuinely want B600 (hero subtitles, prominent body) opt in instead. Don't flip blind: audit a sample of feature screens first to confirm B500 is the majority case before changing the default for everyone.
...
- **Bust treatment for other players — static stamp landed; chip-collapse animation still open.** `BustedStamp` DS primitive (tilted "BUSTED" ink-stamp over the avatar, danger-tinted) shipped 2026-05-22 in `:libraries:ui/components/poker`. `OpponentsRow.OpponentSeat` computes `busted = stack <= 0 && participation != NotDealt && !seatEmpty` and renders the stamp over the avatar with a 0.35α dim on the seat subtree (slightly stronger than the folded dim, so a busted-folded seat reads as busted, not folded). Bots auto-rebuy at the start of the next hand, so the stamp shows for the end-of-hand showdown moment only. New `OpponentsRowPreview_BustedOpponent` exercises the state. Remaining: the "chip stack collapses" half — a small animation when the stack number hits zero would land the moment harder than the static stamp alone. Worth picking up next time the OpponentsRow area is open.
