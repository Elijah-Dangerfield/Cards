...
- `[P1]` **Wire Home's friends strip to the real friend graph.** The Profile inbox is live (`FriendRepository.observeIncomingRequests` / accept / decline → `FriendRequestsSection` on `ProfileScreen.kt`). The Home `FriendsStrip` still runs on fake data: its `pendingRequests` badge and `List<FriendOnline>` come from `defaultOnlineFriends()` in `HomeScreenContent`, not the repository. Drive the `pendingRequests` badge from `FriendRepository.observeIncomingRequests().size` (real today) and have the "N friend requests" tap route into the Profile inbox; the online-friends list stays stubbed until the presence signal lands.
