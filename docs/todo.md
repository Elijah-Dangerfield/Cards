...
- **Stop reaching past the DS for Material primitives.** Several callsites are pulling `androidx.compose.material.icons.Icons.*` + `androidx.compose.material3.{Icon, IconButton}` directly instead of `:libraries:ui`'s `Icon` / `IconButton` / `Icons`. Result: inconsistent tint, sizing, and tap target; one-offs leak Material's defaults into screens that otherwise honor the DS. The PlayPokerScreen TopBar has been migrated — remaining callers are the in-flight Stats info-button refactor that just swapped *from* the DS *to* Material, plus anywhere `Icons.AutoMirrored.Filled.*` / `Icons.Default.*` / `Icons.Outlined.*` is still imported. The fix is routing through the DS components and adding any missing icon entries to `:libraries:ui:components:icon:Icons`. **Out of scope:** redesigning the DS icon set; this is migration, not authoring.
...
- **Edit profile on the fallback pack — surface a clear retry.** If `/v1/avatars` has never landed (first launch, offline), EditProfile shows [FALLBACK_AVATAR_PACK](../libraries/identity/impl/src/commonMain/kotlin/com/cards/libraries/identity/impl/profile/ProfileRepositoryImpl.kt) (8-emoji subset of the server starter, guaranteed-valid via the append-only invariant). The picker works, but if the *user* is also offline, the patchMe save itself fails with a generic error. When patchMe fails with a network error *and* the visible pack is the fallback, swap the error copy to "couldn't reach the server — your avatar wasn't saved" with a Retry CTA, and keep the optimistic local row so the avatar appears immediately even though the server hasn't accepted it yet. Pairs with the `Profile.Fallback` per-feature audit in [developer-todo.md](./developer-todo.md) (Edit Profile is one of the surfaces that needs a per-screen behavior decision).
