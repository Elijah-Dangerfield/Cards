...
- `[P0]` **Add `version: Int` discriminator to every persisted `GameEvent` shape.** Per the eval: "schema evolution on events is forever." Bake the discriminator into the JSONB envelope (`{"v": 1, "type": "...", "payload": {...}}`) from the very first row that lands so a future v2 reader can fork on it without rewriting history. **Acceptance:** envelope shape is committed in code + a `EventEnvelopeTest` pinning the v1 layout. **Files / hints:** add the serialization helpers next to `GameEvent` itself in `:libraries:gameplay`. **Out of scope:** any actual v2 event — this is just the future-proofing.
