...
## AUTH-30 [P0] — Offline cold boot destroys the guest session it can't reach the server to verify

**Problem:** A guest who cold-boots on captive-portal / dead-but-connected WiFi permanently loses their account. With the backend unreachable, `resolve` exhausts its 5 attempts, `GuestSessionHealer` declares the cached anonymous profile unrecoverable, and the teardown calls `gateway.signOut()` (clearing storage **and** the `SessionMirrorStore` that exists to prevent exactly this) plus `profileCache.clear()` (nulling `localId`). `SessionExpired` is sticky, so the recovery screen's "Try again" re-resolves against the files just deleted and fails forever; the only exit is Logout, which the copy itself warns clears chips, XP and history for good. Reconnecting does not undo it. The client also logs "Session rejected by auth server" while `is_offline=true` — the auth server was never reached.

**Acceptance:** An offline cold boot with a cached guest profile never tears down the session or clears the cache — the guest keeps their identity and gets an honest offline surface, not "we lost your guest session". A transient (unreachable-backend) refresh failure is settled distinctly from a server-confirmed dead session. Regression tests cover the production case that no test covers today: `offline = true` **and** a cached `Profile.Authenticated`.

**Desired behavior:** on an unreachable-backend boot the guest keeps their cached identity and enters the app in offline mode behind the honest offline banner, re-attempting resolution on the next `ConnectivityRegained` / foreground edge — never the recovery screen. This is the reporter's own steer ("ideally we just let them okay offline").

**Guardrail (don't regress AUTH-19/AUTH-25):** teardown must still fire for a *server-confirmed* dead session (a real 401/403) — only an unreachable/timeout failure gets spared. `classifyRefreshFailure` is exactly that seam; the fix must keep the healer's "never mint over / never destroy a real account" protection intact, only stop it from triggering on a network timeout the client mistook for a rejection.

**Hints:** Case `docs/agent/feedback-cases/300814df036746f795d3b50ff0fc47ef.md`; Sentry CARDS-BT / CARDS-BV. Durable fix is to preserve the `RefreshFailure` cause through `RealSupabaseAuthGateway.currentStatus()` (currently `RefreshFailure(cause = null)`) and refuse to settle a transient failure as a dead session — `SupabaseAuthRepositoryImpl.settleUnauthenticatedLocked()` emits `Reason.None`, indistinguishable from a fresh install. `RefreshFailureClassifier.classifyRefreshFailure` already exists and is already used by the token path; wire it here. **Reordering the `isOffline` gate in `GuestSessionHealer.healIfStranded` (`GuestSessionHealer.kt:133-153`, where `stopForRecovery` is chosen before offline is consulted) is necessary but NOT sufficient** — `AppStateImpl` starts `initialValue = false` (optimistically online), so on a faster boot the flag may not have resolved yet. Siblings `AuthReResolver.kt:53` and `StrandedIdentityDetector.kt:64` already gate on offline first. Note `ProfileRepositoryImplTest.sessionExpired_dropsTheGhost_…` currently pins the cache wipe as correct behaviour and will need revisiting.
