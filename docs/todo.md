...
- **`[P3]` "Open to anyone" discovery is range-overlap only — a host at off-band stakes may sit unfound.** **Reality check (2026-06-23):** matching is *not* exact — the Find screen is a free-form **range** slider (`PublicFindScreen`, 100–100k scale, no canonical snapping; default ≈ 18,100–60,000) and `findOrJoinPublic` matches `room.buyIn in minBuyIn..maxBuyIn`. So an Open table is discoverable by any searcher whose range spans its buy-in. The only gap: a host who picks stakes *outside* common search bands (e.g. 3,000, below the default floor) won't be found unless a searcher widens/lowers their range — inherent to range matching, not a bug. **Not a launch blocker.** **Future UX (the real improvement, applies to Public too):** the flow today auto-joins the *first* server-side match; a nicer model the owner floated is to **present found tables and let the user accept a buy-in** before joining (a results/confirm step in the Searching flow) rather than silently seating them. That's a Searching-screen redesign, not a stakes-snapping hack. **Don't** snap host-chosen stakes to canonical tiers — it overrides a deliberate host choice for a problem the range slider already mostly solves. Noted in [decisions.md](./decisions.md) (2026-06-23 "Open to anyone").
...
- `[P1]` **Buy-in / stack / re-buy mechanic — settle MP results to the real wallet.** **→ Full implementation plan: [`docs/plans/mp-chip-buyin-economy.md`](./plans/mp-chip-buyin-economy.md).** Spec [§4.1](./product/product-spec.md#wallet-stack--buy-ins). Today MP table stacks are ephemeral and never touch the chip wallet, so a player who wins hands sees no change in their real balance (confirmed 2026-06-19 playtest), and the create-screen buy-in (PR #61) only sets table stakes. The decided model is the **cash-game escrow**: sit-down debits the buy-in; leaving credits the *current* stack back (settle on **every** exit path); re-buy on bust; leave framed as cash-out. **The server engine is now SALVAGED (dormant) onto `feat/public-matchmaking`** (commit `a8a0527b`, 2026-06-23): `TableSessionService`/`WalletLedger`/`table_sessions` (as `V67`) + Testcontainers tests, all green. **`feat/mp-chip-economy` has been deleted** (local + remote) after the engine salvage — it was stale/pre-refactor with a V60 clash; the engine is the only part worth keeping and it's now on the feature branch. What's left is the wiring + client + subsidy: `StakeTier`-on-`Room`, `SitDown`/`Rebuy` socket frames + DI, StartHand "funded players only" flip, leave/disconnect cash-out, leave-confirmation UX, reconcile with **develop's own** rebuy UX (don't re-import the branch's), and the matchmaking disclosed-bot subsidy. See the banner in [`mp-chip-buyin-economy.md`](./plans/mp-chip-buyin-economy.md). **Depends on:** B0.
