...
- `[P1]` **Degraded "account-creation pending" UX (offline guest onboarding).** Deferred creation self-heals — `DefaultGuestAccountCreator` persists the chosen identity (`PendingGuestAccountStore`) and resumes on the next launch; onboarding finish sets `OnboardingState.creationFailed`. Missing is the *user-facing* degraded experience while pending: (a) a Home dialog/banner "we're finishing setting up your account — some features are limited, we'll keep retrying"; (b) a manual **Retry** affordance on Profile/Settings near `SaveProgressBanner`; (c) **connectivity-flip retry** (observe `LocalAppState.isOffline` → `GuestAccountCreator.retry()`) so it heals mid-session, not just on relaunch; (d) a client-side **multiplayer gate** — `PlayMultiplayerFeatureEntryPoint` spins forever on `AuthState.Unauthenticated`; show "finish setup / sign in" instead of hanging. *(proposed 2026-06-09)*
  **Hints:** observe `GuestAccountCreator.state` (app-scoped, globally injectable); dialog via the `HomeEvent.OpenWelcomeDialog`→`WelcomeDialogRoute` pattern; gate in `PlayMultiplayerFeatureEntryPoint` (~line 88).
