...
- **Welcome dialog must always fire on first-ever launch, independent of grant signals.** Repro 2026-05-23: fresh install, no welcome dialog appeared. Likely root cause: when the day-0 +500 grant was removed in [b7cbf4a](https://github.com/) (signup day is now starter-only; daily grants start day 1), the dialog's gate at [HomeViewModel.kt:85](../features/home/impl/src/commonMain/kotlin/com/cards/features/home/impl/HomeViewModel.kt#L85) still depends on `chips != null`. If chips don't hydrate from the server in time on a fresh install (slow network, anonymous sign-in still resolving, etc.) the four-gate `.first { payload() != null }` predicate never trips and the dialog is silently skipped.

  **Acceptance:** On any install where `appData.isFirstEverSession() && !hasSeenStarterWelcome && profile is Authenticated`, the welcome dialog fires — even if chips haven't hydrated yet. The dialog should render with a graceful chip placeholder (or fetch chips lazily after open) rather than blocking on the chip gate. Verify by force-quitting after fresh sign-in *before* `/v1/me/wallet` returns and relaunching: dialog should still appear.

  **Files / hints:** [HomeViewModel.kt:85](../features/home/impl/src/commonMain/kotlin/com/cards/features/home/impl/HomeViewModel.kt#L85) (`observeWelcomeGate` + `WelcomeGate.payload()`), [WelcomeDialog.kt](../features/home/impl/src/commonMain/kotlin/com/cards/features/home/impl/WelcomeDialog.kt), [AppLifecycleTracker.kt](../libraries/cards/impl/src/commonMain/kotlin/com/cards/libraries/cards/impl/AppLifecycleTracker.kt) (where `lastSessionEndedAt` is set — note it only writes on `onBackground`, so the "first launch" signal stays true until the user *leaves* the app at least once). The starter-grant copy at [WelcomeDialog.kt:126](../features/home/impl/src/commonMain/kotlin/com/cards/features/home/impl/WelcomeDialog.kt#L126) already mentions the welcome-week bonus — that copy is the "we gave you something" line; the dialog just needs to actually surface.

  **Out of scope:** changing the chip-grant schedule or adding a separate "welcome" grant. The starter 10K already lands on signup — the issue is purely the dialog gate, not what we give.
...
- **`unlock_only` flag — wire the earned-grant path + render Earned in My Items.** [product-spec.md §4.2](./product/product-spec.md#42-the-unlock-only-catalog) is structurally load-bearing: legendary achievement cosmetics, league-tier cosmetics, RFT cosmetics, achievement-chain cosmetics are **never in the shop, ever.** Shop catalog and unlock catalog must be disjoint. **Foundations are in place — there is no separate Trophy Case surface; earned items land in My Items.** Remaining work:
   ...
   - **Client UI:** `MyItemsScreen` needs to render an "Earned" badge + earn-source (achievement / league name) on rows where `acquisitionSource = Earned`, plus a celebratory unlock dialog *at the moment of earning* — that's where prestige lives, not on the shelf. Shop search results for an earned item should render as "Owned" so the user never sees it as missing.
...
### Navigation — cross-tab back-stack pollution
- **Edit Profile → Shop deep-link strands the Profile tab.** Repro 2026-05-23: Profile tab → Edit Profile → tap a locked avatar pack's "Get in shop" → routes to `ShopRoute(pendingProductId = …)` which opens the purchase sheet. Close the sheet. Now tap the Profile tab in the bottom bar: nothing happens (button visually depresses but the screen doesn't change). Symptom: the Profile tab is unreachable until the user back-stacks out manually.

  **Smell:** the "Get in shop" navigation from Edit Profile is a plain `router.navigate(ShopRoute(...))` ([ProfileFeatureEntryPoint.kt:184](../features/profile/impl/src/commonMain/kotlin/com/cards/features/profile/impl/ProfileFeatureEntryPoint.kt#L184)) — it doesn't go through the bottom-bar's `popUpTo(HomeRoute) { saveState = true }` + `restoreState` flow. So the back stack becomes `[Home, Profile, EditProfile, Shop]` with Shop sitting on top of the Profile tab's stack, not on top of a saved Shop-tab stack. When the user then taps the Profile tab, the bottom-bar handler at [App.kt:230-249](../apps/compose/src/commonMain/kotlin/com/cards/App.kt#L230) navigates to `ProfileRoute()` with `popUpTo(HomeRoute) { saveState = true }` and `restoreState = true` — the `saveState` saves the *current* (Profile-tab-rooted Shop) stack under Shop's key, and `restoreState` then looks for a previously-saved Profile stack that doesn't exist. End result depends on Navigation-Compose internals but the user-visible outcome is "tap does nothing."

  **Fix sketch — two options, pick one:**
  1. **Route cross-tab navigation through the bottom-bar flow.** When Edit Profile needs to open Shop, use the same `popUpTo(HomeRoute) { saveState = true }` + `launchSingleTop` + `restoreState` recipe the bottom bar uses, so the Profile tab's stack is saved and Shop opens on the Shop tab's stack with the `pendingProductId` argument. Means lifting the bottom-bar nav options into a shared helper (e.g. `Router.switchTab(route)`).
  2. **Detect cross-tab destinations on tab tap.** In the bottom-bar handler, instead of relying on `hasRoute<ProfileRoute>()` alone, walk the current back stack to figure out which tab the current destination *belongs to* (via `tabString()` and back-stack ancestry) and use that to drive the saveState/restoreState calls. More defensive but doesn't fix the root issue of one tab's nav graph hosting another tab's route.

  Option 1 is cleaner. Option 2 is the fallback if the Shop destination has reasons to live in multiple tab graphs.

  **Acceptance:** Profile tab → Edit Profile → "Get in shop" → close sheet → tap Profile tab → lands on the Profile screen. Repeat with Home tab → … → Shop → tap Home: lands on Home. No tab becomes "stuck."

  **Files / hints:** [App.kt:215-251](../apps/compose/src/commonMain/kotlin/com/cards/App.kt#L215) (bottom-bar `onItemClick` + tab selection), [App.kt:322-327](../apps/compose/src/commonMain/kotlin/com/cards/App.kt#L322) (`tabString()` classifier), [ProfileFeatureEntryPoint.kt:184](../features/profile/impl/src/commonMain/kotlin/com/cards/features/profile/impl/ProfileFeatureEntryPoint.kt#L184) (the cross-tab nav site). The `Router` abstraction may need a `switchTab(route)` to encapsulate the savedState/restoreState dance.

  **Out of scope:** redesigning the shop's `pendingProductId` deep link — the parameter passing works, it's the navigation graph placement that's wrong.
...
- **Swipe-fold cancel leaves cards off-screen.** Repro 2026-05-23: swipe up past the commit threshold, the `SwipeFoldConfirmDialog` appears (first-trigger UX), tap Cancel — the dialog dismisses but the hole cards stay off-screen because nothing resets `dragOffsetY`. Root cause at [PlayerArea.kt:174-216](../features/room/impl/src/commonMain/kotlin/com/cards/features/room/impl/PlayerArea.kt#L174): when the drag commits, the gesture's `onDragEnd` runs `dragOffsetY.animateTo(-foldFlightPx)` (~400dp upward) *and* calls `onSwipeFold()`. The cancel path on the confirm dialog never reaches back into PlayerArea to spring the offset back to 0. The `LaunchedEffect(swipeFoldEnabled)` reset at line 140 only fires when `swipeFoldEnabled` flips — but the human is still on-turn after a cancel, so that gate stays true and the reset is skipped.

  **Fix sketch:** the confirm-dialog cancel needs to feed back into the gesture state. Options:
  1. Hoist `dragOffsetY` into the calling screen / VM so the cancel handler can reset it directly.
  2. Add a `resetSwipeFoldOffset` event from `PlayPokerViewModel` that PlayerArea observes via `LaunchedEffect` and snaps/animates the offset back to 0.
  3. Don't commit the off-screen animation until the user *confirms* — show the confirm dialog with the cards still in their dragged position (or sprung back), and only run the flight animation after the confirm. Cleanest semantics; the toss is part of the commitment, not part of the question.

  Option 3 is the most honest. Worth a quick design call before picking.

  **Acceptance:** swipe past threshold → confirm dialog appears → tap Cancel → cards spring back into their hole-card slots within a beat. No residual translation. Re-running the gesture after cancel works the same as the first time.

  **Files / hints:** [PlayerArea.kt:128-234](../features/room/impl/src/commonMain/kotlin/com/cards/features/room/impl/PlayerArea.kt#L128) (the `Animatable` + `detectVerticalDragGestures` block — the `onSwipeFold()` callback is the bridge to the VM/dialog), [PlayPokerViewModel.kt](../features/room/impl/src/commonMain/kotlin/com/cards/features/room/impl/PlayPokerViewModel.kt) (`AcknowledgeSwipeFoldGesture` / Fold intent dispatch + `SwipeFoldConfirmDialog` wiring).
...
- **Locked-pack lock badge is clipped by the circular tile.** Repro 2026-05-23: the small 🔒 chip in the bottom-right of locked avatar tiles in the avatar-pack picker is visibly cut off — the chip sits at `Alignment.BottomEnd` inside a `BoxWithConstraints` that has `.clip(CircleShape)` applied, so the badge's bottom-right corner extends outside the circle and gets clipped away with the tile background.

  **Fix sketch:** lift the lock badge out of the clipped circle. Wrap the avatar tile in a small unclipped `Box`, paint the circular tile underneath (the existing `BoxWithConstraints` content), and overlay the lock badge as a sibling positioned at `Alignment.BottomEnd` of the outer box with a small negative offset (or just inset enough that the whole 18dp chip sits inside the parent bounds). Don't move the clip off the inner circle — the avatar emoji + alpha overlay still need to be clipped to the tile shape.

  **Acceptance:** on every locked tile in the pack picker, the full lock badge (background + 🔒 glyph) is visible, including its bottom-right corner. No regression to the unlocked-tile visual.

  **Files / hints:** [EditProfileScreen.kt:370](../features/profile/impl/src/commonMain/kotlin/com/cards/features/profile/impl/edit/EditProfileScreen.kt#L370) `AvatarTile` — the lock badge is the `if (isLocked) Box(...)` block at line ~405.
