...
- `[P1]` **Finish the auth-outcome classifier — VerifyEmail path + optional typed outcome.** The authoritative net-new signal now exists end-to-end (server `/v1/me` `isNewAccount` → `ProfileRepository.resolveIsNewAccount()` / `observeAccountJustCreated()`, latched race-safe), and **onboarding routing + the Home starter-grant welcome** already run on it instead of `walletJustCreated` (AUTH-22 ph1–3 + AUTH-23, shipped this session). Remaining: (a) rewire `VerifyEmailViewModel.isBrandNewAccount()` the same way (needs a small DI change to inject the profile repo — it currently injects only `ChipsRepository`); (b) optional polish — formalize a single typed `AuthOutcome` (`SignedUp`/`SignedIn`/`Linked`) returned from the auth layer so the three cases are a type, not reconstructed per-call-site. `walletJustCreated` can then be retired from `ChipsRepository`.
  **Acceptance:** no auth path (Apple/Google/email, onboarding/verify/claim) classifies new-vs-returning from `walletJustCreated`; VerifyEmail routes brand-new accounts through onboarding via the authoritative signal.
  **Hints:** `VerifyEmailViewModel:154` `isBrandNewAccount()`; ties to AUTH-19 (stable identity). See docs/decisions.md "Deterministic auth-outcome state machine (AUTH-22)".
