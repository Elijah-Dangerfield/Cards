...
- `[P1]` **MP-17 — Remaining MP terminal edge cases (the all-in-leaver burn is fixed).** Shipped: an all-in player who left mid-hand no longer burns their won pot — `GameSession.deferSettlementIfAllInLive` defers their cash-out and `departedSettlements` settles their resolved stack on showdown (`MatchOverEdgeCasesTest`). Also green: multi-way bust to one survivor (`threeWayBustToOneSurvivor…`), ruling out the audit's "zero-survivor wedge" (chip conservation always leaves a survivor). **Still open:**
  - **Sole all-in contender after a fold wedges (found here).** `GameEngine.forfeitSeat` only awards by fold when the lone remaining contender is *not* all-in (line ~227, `&& != AllIn`). If two players are all-in heads-up and one leaves, the fold leaves a sole all-in contender that's never awarded → the hand never completes (so its `departedSettlements` never fire either). Awarding a sole contender even when all-in (everyone else folded → board irrelevant) is correct; verify it doesn't regress the multi-all-in showdown path.
  - **Untested:** 3→2→1 staged collapse, simultaneous mid-hand leaves, reaper grace expiring mid-hand.
  **Hints:** `GameEngine.forfeitSeat` (line ~227), `GameSession.forfeitSeat`, `MatchOverEdgeCasesTest` is the template (the both-leave case exercises the sole-all-in wedge).
