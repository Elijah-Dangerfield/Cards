...
- `[P1]` **Achievements → server (earned-set sync) — Phase 3 Slice 2.** Earned achievements are still client-only, so they're **lost on account switch** (the user-scoped clear wipes the achievement DAOs with no re-hydrate) and on reinstall. XP went server-authoritative in Slice 1; achievements are the matching gap. **Acceptance:** `achievements_earned` server table + sync mirroring XP/wallet; earned set survives switch/reinstall; criteria evaluation + the cross-hand counters stay client-local; keep the existing `/v1/me/grants/achievement/{id}` cosmetic endpoint. **Hints:** clone `V52__xp_progression.sql` / `PostgresProgressionRepository` (upsert idempotent on `(user_id, achievement_id)`); client mirrors `ProgressionRepositoryImpl.sync`. *(proposed 2026-06-14)*

### Progression & XP (server)

- `[P2]` **Graduate lifetime hand counters to the server (decision + do).** `total_xp` is server-authoritative (Slice 1) but the `progression` hand counters (handsPlayed/won/folded/lostAtShowdown/botHandsPlayed) are still client-local — they reset on a switch and aren't re-hydrated, so the stats page shows correct XP/level but **zeroed hand counts** after switching accounts. **Decision:** add counter columns to `user_progression` + carry them in the sync, or document accept-reset. **Lean: graduate** — monotonic + tiny, and the Phase 4.2 MP-achievement floor wants a server `hands_finished` signal anyway. *(proposed 2026-06-14)*
