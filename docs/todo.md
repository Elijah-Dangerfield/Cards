...
- **`unlock_only` flag on products + Trophy Case surface.** [product-spec.md §4.2](./product/product-spec.md#42-the-unlock-only-catalog) is structurally load-bearing: legendary achievement cosmetics, league-tier cosmetics, RFT cosmetics, achievement-chain cosmetics are **never in the shop, ever.** The shop catalog and the unlock catalog must be *disjoint sets.* Server side landed 2026-05-21: `unlock_only BOOLEAN NOT NULL DEFAULT FALSE` column on `products` (migration V10), `ProductsTable.unlockOnly` Exposed column, and `PostgresProductCatalogSource.read` filters `WHERE unlock_only = FALSE` so the shop never surfaces them. Remaining work:
   ...
   - **Client — direction pivot 2026-05-21; trophy-case scaffold scrapped 2026-05-22:** the separate Trophy Case surface is gone. `TrophyCaseRoute`, `TrophyCaseScreen`, the profile `screen<TrophyCaseRoute>` registration, the `onOpenTrophyCase` callback chain on `ProfileScreen`, and the ListSection row that pointed at it were all removed; `MyItems`'s supportingText was widened to "Owned card backs, felts, emotes, titles, earned trophies" so the surface description reads as the merged shelf. Remaining client work: extend the inventory model with an `acquisitionSource: Purchased | Earned` field (server already knows — the row's writer knows whether it came from a purchase, an achievement reward, or a league finish), then update `MyItemsScreen` to section / chip-filter by source. The "trophy" feeling is carried by (a) an "Earned" badge on the row plus the earn-source (achievement / league name), and (b) a celebratory unlock dialog *at the moment of earning* — that's where prestige actually lives, not on the shelf afterward. The shop must also read this: an unlock-only item the user has earned should render as "Owned" in shop searches, not appear missing. **Bonus design lever — "unlock and buy":** a product can have both an earnable variant (achieve X to unlock) and a purchasable variant of related items. Earning the first one is a taste-test that pre-qualifies the user as a buyer for the rest of the family.
...
- **Purchase snackbar polish — emoji bubble landed 2026-05-22; inline "Equip" action still open.** Snackbar DS gained an optional `emoji: String?` parameter (threaded through `showSnackBar(...)` → `SnackbarMessage` → `PodawanSnackbarVisuals` → `Snackbar` composable). When non-null it renders as a 36dp circular `surfaceTertiary`-backed bubble pinned right-of-content, before the dismiss button — NOT the over-the-lip dialog-style bubble. `ShopFeatureEntryPoint` now passes `event.offer.iconEmoji` on `RedeemSucceeded` + `AlreadyOwned`, and a default `🪙` glyph on chip-pack `IapPurchaseOutcome.Success` / `AlreadyOwned`. **Still open:** the inline "Equip" action half — needs the equippability discriminator first (per the avatar picker bullet above: V1 has felts / card backs / table themes / titles / emote packs / tools / avatars all under `chip_offer` with no `kind` field, so the snackbar currently has no way to decide whether to show "Equip"). Defer until a non-avatar equippable surface forces the discriminator question. The action slot in the snackbar already exists; once equippability is sortable, it's a one-line wire-up at the shop entry point.
...
- **Implement buy-in / stack / re-buy mechanic.** Spec landed in [product-spec.md §4.1 → Wallet, stack & buy-ins](./product/product-spec.md#wallet-stack--buy-ins); engineering still needs to do it. Sketch of the work:
  ...
  - **Bot tables:** stake-tier picker landed 2026-05-21. `StakeTier` enum in `:libraries:gameplay` carries the five spec tiers (Practice 1/2/200, Casual 5/10/1k, Standard 25/50/5k, High 100/200/20k, Premium 500/1k/100k) with `toRoomSettings()` derivation; `PlayBotsRoute` carries the chosen tier as a string; `SoloBotsPokerSessionFactory` consumes it and threads it into `LocalBotsSession.settings`. The dialog renders the picker alongside the seat-count picker and shows the current tier's "blinds X / Y · Z chip stack" subtitle. Remaining: V1 mechanic itself (rebuy on bust=0, stack-vs-wallet display, sit-out toggle) and the MP buy-in flow — both still open as their own sub-bullets above/below.
