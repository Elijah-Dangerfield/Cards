...
- `[P1]` **MP-17 — Fix the all-in-leaver chip burn (diagnosed; reproducing test ready).** An all-in player who leaves mid-hand has their committed chips **burned**: the `MemberLeft` handler cashes them out at `stackFor` = 0 (committed chips excluded — correct for a *folded* leaver), but the engine deliberately keeps an all-in seat live so it keeps its showdown right (`forfeitAllInSeat_isNoOp`). If that all-in then wins, the pot lands on a seat already cashed out at 0 and ~5,050 chips vanish (a heads-up table settles at 14,950, not 20,000). **Fix:** deferred settlement — don't cash an all-in leaver out at 0; settle their real resolved stack when the committed hand completes (a hand-completion seam with wallet access). Folding them instead is wrong — it would rob a briefly-*disconnected* all-in player of a winning hand. The reproducing test is `MatchOverEdgeCasesTest.allInPlayerLeavesMidHand_forfeitsThePot_chipsStillConserve` (currently `@Ignore`d — un-ignore when fixing).
  **Acceptance:** the ignored test passes (chips conserve to 20,000); a leaver's free stack still cashes out immediately, only the all-in committed portion defers.
  **Hints:** `RoomSocketRoutes` `MemberLeft` handler (~246, has the `GameSession` via `peek`), `GameSession` (a `departedSettlements` SharedFlow drained at the hand-completion sites ~321/536, mirroring `matchOverEvents`), `SeatStack.stackFor`. Already covered + green: multi-way bust to one survivor (`threeWayBustToOneSurvivor…`), so the "zero-survivor wedge" theory is ruled out (chip conservation always leaves a survivor).
  **Still untested (fold into the fix or a follow-up):** 3→2→1 staged collapse, simultaneous mid-hand leaves, reaper grace expiring mid-hand.
