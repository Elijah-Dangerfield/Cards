...
- `[P1]` **Audit existing earnable pairings + redirect new ones toward volume / skill / placement.** Survey what's in [`EarnableCosmetics.kt`](../libraries/cards/src/commonMain/kotlin/com/cards/libraries/cards/EarnableCosmetics.kt) today (18 pairings; 4 vanity titles, 0 single-showdown RNG after the 2026-05-29 RNG-title retire); per pairing, decide keep / raise-the-bar / retire. New pairings come in three tiers based on cosmetic type:
...
- `[P1]` **Expand OpenTelemetry span coverage on the server — remaining sub-spans.** The OTel SDK + exporter wiring (OTLP/HTTP when `OTEL_EXPORTER_OTLP_ENDPOINT` is set, stdout via `LoggingSpanExporter` otherwise) shipped 2026-05-29 in `apps/server/.../plugins/Telemetry.kt`, alongside an outer `submit_intent` span on the `RoomClientFrame.SubmitIntent` path and an inner `engine.apply_intent` span around `GameEngine.applyIntent` inside `GameSession.applyIntent`. Both carry `room.code` / `user.id` / `intent.type` / `session.id` / `hand.number` attributes (see `SpanAttrs.kt`). **Remaining work to match the original spec's full breakdown:** (a) split out `validate-intent` and `state-mutate` child spans inside `GameSession.applyIntent` — the outer-span body currently includes both phases without a finer breakdown; (b) add `broadcast` + per-recipient `ws-send` spans on the publisher → `sendJson` path. The latter requires propagating the `submit_intent` OTel context through the `StateFlow → flatMapLatest → merge → map` chain in [`RoomSocketRoutes.kt`](../apps/server/src/main/kotlin/com/cards/server/routes/RoomSocketRoutes.kt) — `asContextElement()` is the primitive, but the publisher coroutine runs in its own scope per subscriber so the context handoff isn't trivial. Also park: parity spans on `StartHand` / `RequestNextHand` (today only SubmitIntent is instrumented).
