...
- **`RoundedCornerShape(12.dp)` repeats as a hand-tuned literal.** `LobbyScreen`, `RankDetailSheet`, `ProfileScreen`, the new `ConnectionBanner` on `PlayPokerScreen`, and `BarChart` all hand-spell `RoundedCornerShape(12.dp)`. `Radii` already tokenizes `R300` (8.dp) and `R400` (10.dp / `Radii.Banner`) but skips 12.dp, which is the de-facto banner/callout radius in practice. Add `Radii.R500` (12.dp) + a `Radii.Callout` alias, then sweep the literal callsites. Cheap once we own a fix-anchor file open.
