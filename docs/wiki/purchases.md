...
- Rate-limit relaxed grants and log every one with the distinct `.replay` wallet reason, so anything abnormal is visible. (Caveat: the install-lineage widening below can promote a mismatched token to a plain `Valid` grant that skips this path — see the note.)
