...
- Only relax for a **StoreKit-replayed** transaction, never for a receipt blob the client submits directly in the normal buy flow. Apple handing us a replay is not something the client can forge; a pasted receipt is where a "redeem someone else's receipt" attack would live.
- Rate-limit relaxed grants and log every one, so anything abnormal is visible.
