...
**Expected:** The search seats you automatically — onto an existing table with players when one fits your range, else onto a fresh table where the search genuinely waits (and eventually offers bots). There is no manual "pick a table" step (MP-35). Finish line is sit-down: you have a stack at the table. The buy-in is reserved from your wallet (wallet drops by the buy-in, not spent).
...
- Joined-table lobby (ROOM-11 / MP-35): when the search seats you onto a table that already has players, you land on a distinct joined-table screen ("You're in") showing the seat grid with the seated players and a "waiting for more players" / "dealing you in" line — NOT the spinning radar. Once a hand deals you go straight to the live table. (Opening a fresh table with nobody to match still shows the radar.)
