...
- [ ] **Create the Sentry project + drop the DSN into config.** One project covers client + server; tag every event with `platform=ios|android|server` + `release=<version>` so cross-platform issue timelines stay unified. Once the project exists, paste the DSN into the server's secret-store + the client's build-config so the SDKs can self-initialize. Engineering picks up the SDK wiring from [todo.md §C Observability](./todo.md).
