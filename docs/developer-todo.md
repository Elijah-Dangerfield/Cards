...
- [ ] **IAP receipt-validation credentials (Apple + Google).** The native validation code is now engineering work in [todo.md §G Billing & IAP](./todo.md) (BILL-1/2). Those validators are written to stay dormant until you provision store API credentials and set them as Fly secrets on `cards-server-dev` (and `-prod` at launch). Until then only Dev/Fake billing works.
  - **Apple:** ASC → Users and Access → Integrations → **In-App Purchase keys** → generate a key (downloads a `.p8`). Capture the **Key ID**, **Issuer ID**, and your app's **bundle id**. Requires the Paid Apps Agreement (see Launch readiness below) to be active. Then: `fly secrets set APPLE_IAP_KEY_ID=… APPLE_IAP_ISSUER_ID=… APPLE_BUNDLE_ID=… APPLE_IAP_ENVIRONMENT=Sandbox APPLE_IAP_PRIVATE_KEY="$(base64 -i AuthKey_XXX.p8)" -a cards-server-dev` (flip `APPLE_IAP_ENVIRONMENT` to `Production` for the prod app).
  - **Google:** create or reuse a service account with the **Google Play Android Publisher API** enabled, link it in Play Console → Users & permissions with **view financial data / manage orders** rights, and download its JSON key. Then: `fly secrets set GOOGLE_PLAY_SERVICE_ACCOUNT_JSON="$(cat service-account.json)" -a cards-server-dev`. (Different *purpose* from the `PLAY_SERVICE_ACCOUNT_JSON` GitHub secret used for store uploads — may be the same service account if it holds both roles.)
  - Gated on the store-listings + pricing item below (productIds must exist before a receipt can be validated). The secret **names** are the contract BILL-2 reads — keep them in sync if you rename.
...
- [ ] `APPLE_IAP_KEY_ID`, `APPLE_IAP_ISSUER_ID`, `APPLE_BUNDLE_ID`, `APPLE_IAP_ENVIRONMENT`, `APPLE_IAP_PRIVATE_KEY` — App Store Server API receipt validation (BILL-2). See "IAP receipt-validation credentials" above for how to obtain each.
- [ ] `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` — Google Play Developer API receipt validation (BILL-2). Full service-account JSON; see the same item above.
- [ ] **Reconcile the IAP secret names above with what BILL-2 actually reads.** The shipped validators read `APPLE_BUNDLE_ID`, `APPLE_STORE_ENVIRONMENT` (defaults to `Sandbox`), optional `APPLE_APP_APPLE_ID` (numeric app id; enables online revocation checks), `GOOGLE_PLAY_PACKAGE_NAME`, and `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` (`BillingConfig.fromEnv` in `apps/server/.../config/ServerConfig.kt`). Apple verification is offline JWS chain validation against bundled root CAs, so it does NOT read the `APPLE_IAP_KEY_ID` / `APPLE_IAP_ISSUER_ID` / `APPLE_IAP_PRIVATE_KEY` (`.p8`) secrets listed under "IAP receipt-validation credentials" above — those names predate BILL-2 and don't match the code. Set the names the code reads; the `.p8` key set only matters if we later add online App Store Server API calls.
