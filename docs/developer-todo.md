...
- [ ] **Read-path caching policy — pick the per-endpoint behavior for the remaining surfaces.** Shop catalog + avatar catalog (`/v1/avatars`) are both on the session-aware "consistency > accuracy" tier now — disk-persisted, refresh fires on session rollover (`SessionTracker`: cold boot OR ≥15-min-background returns), 7-day max snapshot age, pull-to-refresh / forced refresh always bypasses. See [decisions.md "Session-aware repository refresh"](./decisions.md). Remaining surfaces still on the old `always-network` / inconsistent paths and waiting on a per-endpoint call: My Items / inventory (currently always-network reload), achievements page, profile (`/v1/me` — needs care since it's per-user and auth-state-dependent), chip balance (should stay accuracy-first). Pick the tier per endpoint; engineering reuses the Shop / Avatar pattern for the consistency-first ones. **V1-polish** rather than blocker.
