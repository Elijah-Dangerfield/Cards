...
- [ ] **Dedicated cosmetic for the "100 multiplayer hands" achievement.** Reaching 100 finished multiplayer hands now earns an achievement server-side and grants a cosmetic — but it currently re-uses the existing single-player "grinder" emote pack rather than its own prize. Minting a dedicated multiplayer cosmetic is a content call (what the prize is) plus a one-line code map + a catalog seed once decided; until then the achievement still lands and grants the grinder emote. The grant degrades cleanly if a new product id is added before its catalog entry exists.
