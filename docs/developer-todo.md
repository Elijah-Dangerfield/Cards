...
- [ ] **Read-path caching policy — remaining server-driven reads.** Shop catalog + avatar catalog (`/v1/avatars`) are on the session-aware tier; see [decisions.md "Session-aware repository refresh"](./decisions.md) + the `AGENTS.md` per-resource table for when the pattern applies. The one remaining clean candidate is **`OfflineFirstAppConfigRepository`**: it owns `/v1/app-config`, persists to disk already, and currently refreshes on a fixed 60-min expiry. Migrating to the session-aware trigger (cold boot + ≥15-min-background rollover) would be a small, consistent win and would let us delete the timer plumbing. **Not** candidates per the AGENTS.md table — kept here so they don't get reopened: inventory / equipment / chips (write-through model, correct as-is), profile resolve (auth-driven), rooms (live), progression / achievements / xp-events (local-only ledgers).
