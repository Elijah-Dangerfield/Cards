...
- [x] **Spin up a real prod Fly app + update `ServerInfo.PROD_BASE_URL`.** *(Done as of 2026-07-07: `cards-server-prod` is deployed and release builds point at it — the URL now lives in [`AppEnvironment.kt`](../libraries/core/src/commonMain/kotlin/com/cards/libraries/core/AppEnvironment.kt); `ServerInfo.kt` no longer exists.)*
...
- [ ] **Mint a logs:write-only Grafana Cloud token for client app events (ENG-18).** The approved plan ([plans/client-app-events-otel.md](./plans/client-app-events-otel.md)) ships client events straight to the Grafana Cloud OTLP gateway with credentials hard-coded in the binary (Sentry-DSN precedent) — so it must be a **dedicated Cloud Access Policy token scoped to logs:write only**, never the server's read/write token. In the Grafana Cloud console: Access Policies → new policy (`logs:write` only) → create token; note the OTLP gateway base URL (`https://otlp-gateway-<region>.grafana.net/otlp`) and instance/stack ID. Hand all three values to the ENG-18 worker (they build against placeholder constants until then).
