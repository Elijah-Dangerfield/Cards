...
- [x] **Mint a logs:write-only Grafana Cloud token for client app events (ENG-18) — done 2026-07-11.** Token minted (logs+metrics+traces write); credentials are build-time injected via `loadTelemetryMetadata` (local.properties `grafana.*` keys locally, `GRAFANA_*` GitHub secrets in beta/release workflows — set) — never committed, the repo is public and push protection blocks it. Pipe verified end-to-end against dev Loki the same day (correlation + offline drill; see decisions/wiki).
