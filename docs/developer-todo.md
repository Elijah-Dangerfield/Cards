...
- [ ] **Read-path caching policy — pick the per-endpoint behavior for the remaining surfaces.** The Shop catalog is now the reference implementation for the "consistency > accuracy" tier — disk-persisted, session-aware refresh via `SessionTracker` (cold boot + ≥15-min-background rollover triggers a fresh fetch), 7-day max snapshot age, pull-to-refresh always forces. See [decisions.md "Session-aware repository refresh"](./decisions.md). Remaining surfaces still on the old `always-network` / inconsistent paths and waiting on a per-endpoint call: My Items / inventory (currently always-network reload), achievements page, avatar catalog (`GET /v1/avatars` fires on every Edit Profile visit), profile, chip balance (should stay accuracy-first). Pick the tier per endpoint; engineering reuses the Shop pattern (`SessionTracker` + persisted `Cache<T>` + `observeIsRefreshing`) for the consistency-first ones. **V1-polish** rather than blocker.
