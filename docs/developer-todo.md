...
- [ ] **Server-side IAP receipt validation (was BILL-1).** Today `ShopAction.ConfirmPurchase` drives the local purchase use case and credits chips on success — the server never validates the receipt, so a forged receipt would mint chips. Before any real-money sale: a `POST /v1/billing/redeem` endpoint validates against the Apple App Store Server API / Google Play Developer API, then grants chips through the server wallet ledger (idempotent per store transaction id, precedent `ChipsRepository.addChips(idempotencyKey=…)`). The server skeleton (route, `billing_transactions` table with `UNIQUE(store, order_id)`, idempotent grant, a `ReceiptValidator` interface + fake impl) is buildable now, but the two **real** validators need store API credentials (App Store Server API JWT key + bundle id; Google Play Developer API service account) that don't exist yet — gated on the store-listings item above. Pulled out of `todo.md` because it's genuinely blocked until those credentials exist.
